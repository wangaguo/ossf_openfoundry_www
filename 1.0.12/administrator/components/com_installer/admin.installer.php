...
function uploadPackage( $installerClass, $option, $element, $client ) {
	...
	if (!(bool)ini_get('file_uploads')) {
		HTML_installer::showInstallMessage( $_LANG->_( "THEINSTALLERCANTCONTINUEUPLOAD" ),
	...
	if(!extension_loaded('zlib')) {
		HTML_installer::showInstallMessage( $_LANG->_( "THEINSTALLERCANTCONTINUEZLIB" ),
			$_LANG->_( 'INSTALLER ERROR' ), $installer->returnTo( $option, $element, $client ) );
		exit();
	}
		...
		if (is_writable( $baseDir )) {
			if (move_uploaded_file( $filename, $baseDir . $userfile_name )) {
				if (mosChmod( $baseDir . $userfile_name )) {
					...
					$msg = $_LANG->_( 'FAILEDTOCHANGE' );
				}
			} else {
				$msg = $_LANG->_( 'FAILEDTOMOVE' );
			}
		} else {
			$msg = $_LANG->_( 'UPLOADFAILEDNOWRITEABLE' );
		}
	} else {
		$msg = $_LANG->_( 'UPLOADFAILEDNOEXIST' );
	}
	return false;
}
?>