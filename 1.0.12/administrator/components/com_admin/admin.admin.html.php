...
class HTML_admin_misc {
	...
	*/
	...
	global $_LANG;
		...
		if ($colour) {
			if ($yn) {
				$r = $r ? '<span style="color: green;">'.$_LANG->_( 'ON' ).'</span>' : '<span style="color: red;">'.$_LANG->_( 'OFF' ).'</span>';
			} else {
				$r = $r ? '<span style="color: red;">'.$_LANG->_( 'ON' ).'</span>' : '<span style="color: green;">'.$_LANG->_( 'OFF' ).'</span>';			
			}
	...
	global $mosConfig_absolute_path, $database, $mosConfig_cachepath, $mainframe;
		...
		$tabs->startTab($_LANG->_( 'System Info' ),"system-page");
		?>
		...
		<?php
		$tabs->endTab();
		$tabs->startTab($_LANG->_( 'PHP Info' ),"php-page");
		?>
		<table class="adminform">
		<tr>
			<th colspan="2">
		...
		<?php
		$tabs->endTab();
		$tabs->startTab($_LANG->_( 'Permissions' ),'perms');
		?>
		<table class="adminform">
		<tr>
			<th colspan="2">
					...
					<tr>
						<td>
							...
							<a href="http://www.joomla.org/latest10" target="_blank">
								<?php echo $_LANG->_( 'Lastest Version Info' ); ?></a>
						</td>
					</tr>
				</table>
			</td>
