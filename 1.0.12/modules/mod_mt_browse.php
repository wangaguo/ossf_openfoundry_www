...
echo '<a href="'.sefRelToAbs("index.php?option=com_mtree&task=listcats&cat_id=0&Itemid=$Itemid").'"> '.$_MT_LANG->ROOT."</a>";
echo "</td></tr>";
$level++;
...
foreach( $pathway AS $cid ) {
	...
	echo '<img src="components/com_mtree/img/dtree/joinbottom.gif" border="0" align="left" vspace="0" hspace="0" />';
	echo '<img src="components/com_mtree/img/dtree/folder.gif" align="left" vspace="0" hspace="0" />';
	echo '<a href="'.sefRelToAbs("index.php?option=com_mtree&task=listcats&cat_id=".$mtCat->cat_id."&Itemid=$Itemid").'" class="'.$currentcat_class.'"> '.$mtCat->cat_name."</a>";
	echo "</td></tr>";
	$level++;
}
...
if ( $cat_id > 0 ) {
	...
	echo '<tr><td>';
	echo str_repeat($spacer, $level);
	echo '<img src="components/com_mtree/img/dtree/joinbottom.gif" align="left" vspace="0" hspace="0" />';
	echo '<img src="components/com_mtree/img/dtree/folderopen.gif" align="left" vspace="0" hspace="0" />';
	echo '<a href="'.sefRelToAbs("index.php?option=com_mtree&task=listcats&cat_id=".$mtCat->cat_id."&Itemid=$Itemid").'" class="'.$subcat_class.'"> '.$mtCat->cat_name."</a>";
	echo "</td></tr>";
	$level++;
}
	...
	foreach( $cats AS $cat) {
		...
		if ( $cat->cat_id == $cats[count($cats)-1]->cat_id ) {
			...
			echo '<img src="components/com_mtree/img/dtree/join.gif" align="left" vspace="0" hspace="0" />';
		}
		echo '<img src="components/com_mtree/img/dtree/folder.gif" align="left" vspace="0" hspace="0" />';
		echo '<a href="'.sefRelToAbs("index.php?option=com_mtree&task=listcats&cat_id=".$cat->cat_id."&Itemid=$Itemid").'" class="'.$subcat_class.'"> '.$cat->cat_name;
