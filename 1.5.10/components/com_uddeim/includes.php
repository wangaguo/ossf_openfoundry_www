...
function uddeIMshowNoMessage($type, $filter_user, $filter_unread) {
	if ($filter_user || $filter_unread) {
		echo "<div id='uddeim-overview'><p>";
		switch($type) {
			case 'inbox':	$user = _UDDEIM_NOMESSAGES_FILTERED_FROM;
							$box  = _UDDEIM_NOMESSAGES_FILTERED_INBOX;
							break;
			case 'outbox':	$user = _UDDEIM_NOMESSAGES_FILTERED_TO;
							$box  = _UDDEIM_NOMESSAGES_FILTERED_OUTBOX;
							break;
			case 'archive':	$user = _UDDEIM_NOMESSAGES_FILTERED_FROM;
							$box  = _UDDEIM_NOMESSAGES_FILTERED_ARCHIVE;
							break;
		}
		$text = sprintf(_UDDEIM_NOMESSAGES_FILTERED,
							($filter_unread ? _UDDEIM_NOMESSAGES_FILTERED_UNREAD : ""),
							($filter_user   ? $user : ""),
							$box);
		echo $text;
		echo "</p></div>\n";
		...
		echo "<div id='uddeim-overview'><p>".$text."</p></div>\n";
	}
}
...
function uddeIMprintFilter($myself, $uddeaction, $count, $item_id, $config, $filter_user, $filter_unread) {
	...
	echo "<table border='0' cellpadding='0' callspacing='0' width='100%'><tr><td align='left'>";
	if ($filter_user || $filter_unread)
	...
	echo " <input type='submit' value='"._UDDEIM_FILTER_SUBMIT."' />";
	echo "</form>";
	echo "</td></tr></table>";
	echo "</div>";
}
...
function uddeIMdoAutocomplete($config) {
	...
	if ($config->mootools==1) { // auto
		...
		uddeIMaddScript($pathtosite."/components/com_uddeim/js/mootools-1.2.js");
