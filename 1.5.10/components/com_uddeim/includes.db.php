...
function uddeIMinsertEMNdefaults($myself, $config) {
	...
	$autoforward   = 0;
	$autoforwardid = 0;
	$sql="INSERT INTO #__uddeim_emn (status, popup, public, autoresponder, autoforward, autoforwardid, userid) VALUES (".
			(int)$status.", ".
			(int)$popup.", ".
			(int)$public.", ".
			(int)$autoresponder.", ".
...
function uddeIMgetArchiveCount($myself, $filter_user=0, $filter_unread=0) {
	$database = uddeIMgetDatabase();
...
function uddeIMgetInboxCount($myself, $filter_user=0, $filter_unread=0) {
	$database = uddeIMgetDatabase();
...
// OPT
	...
	if ($filter_user==-1) $filter = " AND a.fromid=0";
	if ($filter_unread) $filter .= " AND a.toread=0";
	$sql = "SELECT count(a.id) FROM #__uddeim AS a WHERE a.toid=".(int)$myself." AND a.totrash=0 AND archived=0".$filter;
	$database->setQuery($sql);
	$total = (int)$database->loadResult();
	return $total;
}
...
function uddeIMgetOutboxCount($myself, $filter_user=0, $filter_unread=0) {
	$database = uddeIMgetDatabase();
...
// OPT 
	...
	if ($filter_unread) $filter .= " AND a.toread=0";
	$sql = "SELECT count(a.id) FROM #__uddeim AS a WHERE a.fromid=".(int)$myself." AND a.totrashoutbox=0".$filter." AND ((a.systemmessage IS NULL) OR (a.systemmessage=''))";
	$database->setQuery($sql);
	$total = (int)$database->loadResult();
	return $total;
}
...
function uddeIMselectInbox($myself, $limitstart, $limit, $config, $filter_user=0, $filter_unread=0) {
	$database = uddeIMgetDatabase();
...
// OPT
	...
	if ($filter_user==-1) $filter = " AND a.fromid=0";
	if ($filter_unread) $filter .= " AND a.toread=0 ";
	$sql = "SELECT a.*, b.".($config->realnames ? "name" : "username")." AS fromname FROM #__uddeim AS a LEFT JOIN #__users AS b ON a.fromid=b.id WHERE a.toid=".(int)$myself." AND a.totrash=0 AND archived=0".$filter." ORDER BY datum DESC LIMIT ".(int)$limitstart.", ".(int)$limit;
	$database->setQuery($sql);
	$value = $database->loadObjectList();
	if (!$value)
		$value = Array();
...
function uddeIMselectInboxMessage($myself, $messageid, $config, $trashed=-1) {
	...
	$sql = "SELECT a.*, b.".($config->realnames ? "name" : "username")." AS fromname FROM #__uddeim AS a LEFT JOIN #__users AS b ON a.fromid=b.id WHERE a.toid=".(int)$myself." AND a.id=".(int)$messageid.$filter;
	$database->setQuery($sql);
	$value = $database->loadObjectList();
	if (!$value)
		$value = Array();
...
function uddeIMselectArchive($myself, $limitstart, $limit, $config, $filter_user=0, $filter_unread=0) {
	$database = uddeIMgetDatabase();
...
// OPT
	...
	if ($filter_user==-1) $filter = " AND a.fromid=0";
	if ($filter_unread) $filter .= " AND a.toread=0 ";
	$sql = "SELECT a.*, b.".($config->realnames ? "name" : "username")." AS fromname FROM #__uddeim AS a FORCE INDEX(datum) LEFT JOIN #__users AS b ON a.fromid=b.id WHERE a.toid=".(int)$myself." AND a.totrash=0 AND archived=1".$filter." ORDER BY datum DESC LIMIT ".(int)$limitstart.", ".(int)$limit;
	$database->setQuery($sql);
	$value = $database->loadObjectList();
	if (!$value)
		$value = Array();
...
function uddeIMselectOutbox($myself, $limitstart, $limit, $config, $filter_user=0, $filter_unread=0) {
	$database = uddeIMgetDatabase();
	...
	if ($filter_user==-1) $filter = " AND a.toid=0";
	if ($filter_unread) $filter .= " AND a.toread=0";
	$sql = "SELECT a.*, b.".($config->realnames ? "name" : "username")." AS toname FROM #__uddeim AS a LEFT JOIN #__users AS b ON a.toid=b.id WHERE a.fromid=".(int)$myself." AND a.totrashoutbox=0".$filter." AND ((a.systemmessage IS NULL) OR (a.systemmessage='')) ORDER BY toread ASC, datum DESC LIMIT ".(int)$limitstart.", ".(int)$limit;
...
function uddeIMselectCBE2buddies($myself, $config) {
	$database = uddeIMgetDatabase();
	$sql="SELECT a.buddyid, a.userid, b.id, b.".($config->realnames ? "name" : "username")." AS displayname FROM #__cbe_buddylist AS a, #__users AS b WHERE b.block=0 AND (((a.userid=".(int)$myself." AND b.id=a.buddyid) OR (a.buddyid=".(int)$myself." AND b.id=a.userid)) AND buddy='1') ORDER by b.".($config->realnames ? "name" : "username");
	$database->setQuery($sql);
	$value = $database->loadObjectList();
	if (!$value)
		$value = Array();
