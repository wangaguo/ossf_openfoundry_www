...
function uddeIMarchive($myself, $item_id, $limit, $limitstart, $cryptpass, $config, $filter_user, $filter_unread) {
	...
	$allmessages = uddeIMselectArchive($myself, $limitstart, $limit, $config, $filter_user, $filter_unread);
	...
	$referlink = "index.php?option=com_uddeim&task=archive&Itemid=".$item_id.$addlink;
	if ($totalarchive>$limit) {
		...
		echo "[<a href='".uddeIMsefRelToAbs("index.php?option=com_uddeim&task=archive&Itemid=".$item_id."&limitstart=0&limit=".$totalarchive.$addlink)."'>"._UDDEIM_SHOWALL."</a>]";
		echo "</div>\n";
	}
