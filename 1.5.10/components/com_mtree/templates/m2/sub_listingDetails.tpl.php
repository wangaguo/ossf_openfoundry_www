...
if( $this->show_actions_rating_fav ) {
	...
	if($this->config->get('show_rating')) {
		...
		if( $this->link->link_votes <= 1 ) {
			echo $this->link->link_votes . " " . strtolower($this->_MT_LANG->VOTE);
		} elseif ($this->link->link_votes > 1 ) {
			echo $this->link->link_votes . " " . strtolower($this->_MT_LANG->VOTES);
		}
		echo '</div>';
		echo '</div>';
	}
	...
	?>
	<div class="favourite">
	<span class="fav-caption"><?php echo $this->_MT_LANG->FAVOURED ?>:</span>
	<div id="fav-count"><?php echo number_format($this->total_favourites,0,'.',',') ?></div><?php 
		if($this->my->id > 0){ 
			if($this->is_user_favourite) {
				?><div id="fav-msg"><a href="#" onclick="return(fav(<?php echo $this->link->link_id ?>,-1))"><?php echo $this->_MT_LANG->REMOVE_FAVOURITE ?></a></div><?php 
			} else {
				?><div id="fav-msg"><a href="#" onclick="return(fav(<?php echo $this->link->link_id ?>,1))"><?php echo $this->_MT_LANG->ADD_AS_FAVOURITE ?></a></div><?php 
				}
		} ?>
	</div><?php
	}
...
if ( !empty($this->mambotBeforeDisplayContent) && $this->mambotBeforeDisplayContent[0] <> '' ) { echo trim( implode( "\n", $this->mambotBeforeDisplayContent ) ); }
echo '<span class="listing-desc">';
