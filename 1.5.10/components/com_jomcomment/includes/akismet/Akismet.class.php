...
class Akismet {
	...
	function getQueryString() {
		...
		foreach($this->comment as $key => $data) {
			if(is_string($data)){ // test $data to make sure it is a string
...
class SocketWriteRead {
	...
	function send() {
		...
		if($fs !== false) {
			...
			// Check for time out.
			if(function_exists('stream_set_timeout') && function_exists('stream_set_timeout')){
				stream_set_timeout($fs, $this->requestTimeOut);
