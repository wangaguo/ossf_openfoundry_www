...
******************************/
	...
	$results		=	$_PLUGINS->trigger( 'onBeforeUserProfileEditDisplay', array( &$user, 1, &$additionalHeadTags ) );
	if ($_PLUGINS->is_errors()) {
		echo "<script type=\"text/javascript\">alert(\"".$_PLUGINS->getErrorMSG()."\"); window.history.go(-1); </script>\n";
		exit();
	}
	...
	if ( count( $additionalHeadTags ) > 0 ) {
		addCbHeadTag( 1, implode( '', $additionalHeadTags ) );
	}
...
<div class="cbEditProfile"><div id="cbEditProfileInner">
	...
	echo HTML_comprofiler::_cbTemplateRender( $user, 'Profile', 'drawEditProfile', array( &$user, $tabcontent, $submitvalue, _UE_CANCEL, $bottomIcons ) );
?>
...
<?php
	...
	function userProfile($user, $option, $submitvalue) {
		...
		if ( $showProfile == 1 ) {
			$additionalHeadTags		=	array();
			$results = $_PLUGINS->trigger( 'onBeforeUserProfileDisplay', array( &$user, 1, $cbUserIsModerator, $cbMyIsModerator, &$additionalHeadTags ) );
			if ($_PLUGINS->is_errors()) {
				echo "<script type=\"text/javascript\">alert(\"".$_PLUGINS->getErrorMSG()."\"); window.history.go(-1); </script>\n";
				exit();
			}
...
' );
			...
			echo HTML_comprofiler::_cbTemplateRender( $user, 'Profile', 'drawProfile', array( &$user, &$userViewTabs ) );
...
<?php
	...
	function registerForm( $option, $emailpass, &$user, $regErrorMSG = null) {
		...
		$additionalHeadTags					=	array();
		$results = $_PLUGINS->trigger( 'onBeforeRegisterFormDisplay', array( &$user, &$additionalHeadTags, $regErrorMSG ) );
		if ($_PLUGINS->is_errors()) {
			echo "<script type=\"text/javascript\">alert(\"".$_PLUGINS->getErrorMSG()."\"); window.history.go(-1); </script>\n";
			exit();
		}
		...
		if($ueConfig['reg_enable_toc']) {
			$tabcontent						.=	"\t<tr>\n"
											.	"\t\t<td>&nbsp;</td>\n<td class='fieldCell'><input type='checkbox' name='acceptedterms' id='acceptedterms' value='1' mosReq='0' mosLabel='"
...
<?php
		...
		echo HTML_comprofiler::_cbTemplateRender( $user, 'RegisterForm', 'drawProfile', array( &$user, $tabcontent, $regFormTag, $introMessage, _LOGIN_REGISTER_TITLE, _REGISTER_TITLE, _UE_REGISTER, $moduleContent, $topIcons, $bottomIcons, $conclusionMessage ) );
/*
//>>>>>>>>>>>>>>>
		if ( $reg_show_login_on_page ) {
			if ( $introMessage ) {
?>
				<div class="componentheading" id="cb_comp_login_register_head"><?php echo $loginOrRegisterTitle; ?></div><div class="cb_comp_outer"><div class="cb_comp_inner">
				<div class="contentpaneopen" id="cb_comp_login_register_content"><?php echo $introMessage; ?></div>
				</div></div>
<?php
			}
...
?>
<div class="componentheading"><?php echo $registerTitle; ?></div><div class="contentpaneopen"><div class="cb_comp_outer"><div class="cb_comp_inner cbRegistration">
<?php
		if ( $topIcons ) {
		...
		echo $regFormTag;		// '<form...>'
?>
<table class="contentpane" id="registrationTable">
<?php
		if ( $introMessage && ( ! $reg_show_login_on_page ) ) {
?>
    <tr>
      <td colspan="2" class="contentpaneopen"><?php echo $introMessage; ?></td>
    </tr>
<?php
		}
