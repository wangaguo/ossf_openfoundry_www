...
class CMSInput {
	...
	{	
		...
		//$this->use_xss_clean	= TRUE; //($CFG->item('global_xss_filtering') === TRUE) ? TRUE : FALSE;
	 ...
	 * @return	string
	 */
	function get($index = '', $default=0, $xss_clean = TRUE)
	{
	    if(cmsVersion() == _CMS_JOOMLA10 || cmsVersion() == _CMS_MAMBO){
	        return mosGetParam($_GET, $index, $default);
		}elseif(cmsVersion() == _CMS_JOOMLA15){
		    return JRequest::getvar($index,$default);
		}
	 ...
	 * @return	string
	 */
	function post($index = '',$default=0, $xss_clean = FALSE)
	{		
	    if(cmsVersion() == _CMS_JOOMLA10){
	        return mosGetParam($_POST, $index, $default);
		}elseif(cmsVersion() == _CMS_JOOMLA15){
		    return JRequest::getvar($index,$default);
		}
