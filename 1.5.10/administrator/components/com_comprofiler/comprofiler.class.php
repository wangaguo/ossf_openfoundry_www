...
class cbTabs extends cbTabHandler {
	...
	*/	
	...
	function getEditTabs( &$user, $postdata = null, $formatting = 'table', $reason = 'edit', $tabbed = true ) {
		...
		foreach( $oTabs AS $oTab ) {
			...
			if ( $tabbed && ( $oContent != '' ) ) {
				...
				$_PLUGINS->trigger( 'onAfterEditATab', array( &$oContent, &$oTab, &$user, &$postdata, $formatting, $reason, $tabbed ) );
	...
	function _getEditTabContents( &$tab, &$user, $formatting = 'table', $reason = 'edit', $prefetchFields = true ) {
		$results				=	'';
