...
function RafCloud_TagCreator($RC_config)
{
	...
	$this->encoding = str_replace('charset=','',_ISO);
	$this->lastTime = $this->getmicrotime();
	$entity_charsets = "ISO-8859-1,ISO-8859-15,UTF-8,cp866,cp1251,cp1252,KOI8-R,BIG5,GB2312,BIG5-HKSCS,Shift_JIS,EUC-JP";
	if (stripos($entity_charsets,$this->encoding)===FALSE) $this->entity_encoding="ISO-8859-1"; else $this->entity_encoding=$this->encoding;
...
function pluginTag($plugin,&$stag,&$metaKey)
{
	...
	if( is_file( $mosConfig_absolute_path ."/administrator/components/com_rafcloud/plugins/".$plugin ) ) 
	{
...
function checkWord($word,&$excluded)
{
	...
	$strlength = mb_strlen($word,$this->encoding);
...
function checkKey($key,&$excluded)
{
	...
	$RC_blacklist=$this->RC_config->getValue("RC_blacklist");
	$strlength = mb_strlen($key,$this->encoding);
	if ($strlength<=$RC_key_min_len) return false;
	if ($strlength>=$RC_key_max_len) return false;
	if (empty ($key)) return false;
	if (!empty($RC_blacklist)) if (stripos($RC_blacklist,$key)!==FALSE) return false;
return true;
}
...
function validate(&$stag)
{
	...
	{
		$stag=html_entity_decode($stag,ENT_QUOTES,$this->entity_encoding);
	}
	//end patch
...
function validate_key(&$skey)
{
	...
	{
		$skey=html_entity_decode($skey,ENT_QUOTES,$this->entity_encoding);
	}
	//end patch
...
function saveWord($word,$counter,$whitelist_on=0,$published=0,$type=1)
{
	...
	$word=trim($word);
	$wordLenght=mb_strlen($word,$this->encoding);
