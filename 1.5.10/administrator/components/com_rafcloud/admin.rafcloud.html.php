...
function settings( $option ,&$RC_config) {
	...
	global $mosConfig_absolute_path,$mosConfig_live_site,$RC_version;
        $tabs = new mosTabs(1);

	$RC_config->setValue("RC_debug", session_id(),'config');
	
	if(is_file($mosConfig_absolute_path."/administrator/components/com_rafcloud/runlog.php"))
		require($mosConfig_absolute_path."/administrator/components/com_rafcloud/runlog.php");
	...
	{
		$old_settings=true;
		include($mosConfig_absolute_path."/administrator/components/com_rafcloud/settings.php");
	}
...
function loadWords()
{
    	document.adminForm.blacklist.value=document.adminForm.blacklist.value+"<?php if(!empty($RC_blacklist)) echo(","); echo str_replace("\n",' ',RC_DEFAULT_BLACKLIST_WORDS) ;?>";
}
</script>
...
<?php
       ...
       $tabs->startTab(RC_TAB_WORDS, "words");
?>
...
<?php
        $tabs->endTab();
       $tabs->startTab(RC_TAB_KEYS, "keys");
?>
  <table width="100%" cellpadding="3" cellspacing="0" class="adminForm" style="border:1px solid #CCCCCC;">
  	<form action="index2.php" method="POST" name="adminForm">
    <tr>
...
<?php
        $tabs->endTab();
	$tabs->startTab(RC_GENERAL, "general");
?>
<table align="left">
...
<?php
        $tabs->endTab();
	$tabs->startTab(RC_TAB_SCHEDULER, "scheduler");
?>
<table>
<tr><TD>
...
<td>
<?php echo RC_DEBUG_MODE; ?> <a target="_blank" href="<?php echo $mosConfig_live_site;?>/index2.php?option=com_rafcloud&debug=1&hash=<?php  echo $RC_debug;?>">DEBUG</a>
</td>
</tr>
<tr>
...
<?php
        $tabs->endTab();
	$tabs->startTab(RC_TAB_TOOLS, "tools");
?>
<table>
<tr>
<td>
<?php echo RC_RUN_DEBUG; ?> <a target="_blank" href="<?php echo $mosConfig_live_site;?>/index2.php?option=com_rafcloud&debug=1&hash=<?php  echo $RC_debug;?>">DEBUG</a>
</td>
</tr>
<tr><TD>
...
<?php
        $tabs->endTab();
	$tabs->startTab("Donations", "donations");
?>
  </form>
<table>
<tr><TD>
<strong>If you like this software and want to push the further development, then please support us with an easy click on the button below:</strong>
