...
class modK2ToolsHelper {
	...
	function getAuthors( & $params) {
		...
		if (count($rows)){
			foreach ($rows as $row) {
				...
				$author->latest->link = JRoute::_(K2HelperRoute::getItemRoute($author->latest->id.':'.$author->latest->alias, $author->latest->catid.':'.$author->latest->categoryalias));
	...
	function getArchive( & $params) {
		...
		$query = "SELECT DISTINCT MONTH(created) as m, YEAR(created) as y FROM #__k2_items  WHERE published=1 AND ( publish_up = ".$db->Quote($nullDate)." OR publish_up <= ".$db->Quote($now)." ) AND ( publish_down = ".$db->Quote($nullDate)." OR publish_down >= ".$db->Quote($now)." ) AND trash=0 AND access<={$aid} ORDER BY created DESC ";
		...
		if (count($rows)){
			...
			foreach ($rows as $row) {
				if ($params->get('archiveItemsCounter')) {
					$row->numOfItems = modK2ToolsHelper::countArchiveItems($row->m, $row->y);
				}
				else {
					$row->numOfItems = '';
				}
	...
	function tagCloud( & $params) {
		...
		$query ="SELECT t.name FROM #__k2_tags as t";
		...
		if (count($rows)) {
			...
			foreach ($cloud as $key=>$value) {
				...
				$tags[$counter]->{'link'} = JRoute::_(K2HelperRoute::getTagRoute($key));
				$counter++;
			}
		...
		if ($level < intval($end_level) || is_null($end_level)) {
			...
			foreach ($rows as $row) {
				...
				if (modK2ToolsHelper::hasChildren($row->id)) {
					...
					$output .= '<li'.$active.'><a href="'.JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.$row->alias)).'"><span>'.$row->name.$row->numOfItems.'</span></a>';
					modK2ToolsHelper::treerecurse($params, $row->id, $level+1);
					$output .= '</li>';
				}
				else {
					$output .= '<li'.$active.'><a href="'.JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.$row->alias)).'"><span>'.$row->name.$row->numOfItems.'</span></a></li>';
				}
			}
			$output .= '</ul>';
		}
	...
	function treeselectbox( & $params, $id = 0, $level = 0) {
		...
		if ($level == 0) {
			echo '
			<form class="k2SelectBox" action="'.JRoute::_('index.php').'" method="get">
		...
		foreach ($rows as $row) {
			...
			if (modK2ToolsHelper::hasChildren($row->id)) {
				echo '<option value="'.JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.$row->alias)).'"'.$selected.'>'.$indent.$row->name.'</option>';
	...
	function breadcrumbs($params) {
		...
		if ($option == 'com_k2' && ( isset ($id) && $id > 0)) {
	...
	function getCategoryPath($catid ) {
		...
		foreach ($rows as $row) {
			array_push($array, '<a href="'.JRoute::_(K2HelperRoute::getCategoryRoute($row->id.':'.$row->alias)).'">'.$row->name.'</a>');
	...
	function countArchiveItems($month, $year) {
	...
	function calendar($params) {
		...
		$cal = new MyCalendar;
		//$cal->setStartDay($params->get('start_day'));
		$cal->setStartDay(1);
...
class MyCalendar extends Calendar {
	...
	function getDateLink($day, $month, $year) {
		...
		if ($result > 0) {
			...
			return JRoute::_('index.php?option=com_k2&view=itemlist&task=date&year='.$year.'&month='.$month.'&day='.$day);
	...
	function getCalendarLink($month, $year) {
		...
		return JURI::root()."index.php?option=com_k2&amp;view=itemlist&amp;task=calendar&amp;month=$month&amp;year=$year";
	}
