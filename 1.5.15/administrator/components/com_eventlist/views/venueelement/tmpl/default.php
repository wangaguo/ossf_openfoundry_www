	...
	<tbody>
		...
		<tr class="<?php echo "row$k"; ?>">
			...
			<td align="left">
				...
				<a style="cursor:pointer" onclick="window.parent.elSelectVenue('<?php echo $row->id; ?>', '<?php echo str_replace( array("'", "\""), array("\\'", ""), $row->venue ); ?>',<?php if($row->reg_area==''){echo '-1';}else{ echo $row->reg_area;}?>);"> 
				<?php echo htmlspecialchars($row->venue, ENT_QUOTES, 'UTF-8'); ?>
				</a>
				<?php
				//*顯示該地址的英文翻譯 若無就不顯示*//
				$en_venue = ELOutput::jf_Tr($row->id,'eventlist_venues');
				if(count($en_venue > 0) && $en_venue['venue'] != ''){
					echo "</br>".$en_venue['venue'];
				}
				?>
