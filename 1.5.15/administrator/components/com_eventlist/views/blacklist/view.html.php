 ...
 * @since 0.9
 */
class EventListViewBlacklist extends JView {
	...
	function display($tpl = null)
	{
		...
		//get vars
		$filter_order		= $mainframe->getUserStateFromRequest( $option.'.blacklist.filter_order', 'filter_order', 	'a.dates', 'cmd' );
		$filter_order_Dir	= $mainframe->getUserStateFromRequest( $option.'.blacklist.filter_order_Dir', 'filter_order_Dir',	'', 'word' );
		$filter_state 		= $mainframe->getUserStateFromRequest( $option.'.blacklist.filter_state', 'filter_state', 	'*', 'word' );
		$filter 			= $mainframe->getUserStateFromRequest( $option.'.blacklist.filter', 'filter', '', 'int' );
		$search 			= $mainframe->getUserStateFromRequest( $option.'.blacklist.search', 'search', '', 'string' );
		...
		JSubMenuHelper::addEntry( JText::_( 'BLACK' ), 'index.php?option=com_eventlist&view=blacklist', true);
		...
		//create the toolbar
		JToolBarHelper::title( JText::_( 'black' ), 'black' );
		JToolBarHelper::spacer();
		JToolBarHelper::deleteList( JText::_( 'DELETEBLACK' ) ,'remove', JText::_( 'DELETE BLACK' ) );
		JToolBarHelper::spacer();
		JToolBarHelper::help( 'el.listblack', true );
