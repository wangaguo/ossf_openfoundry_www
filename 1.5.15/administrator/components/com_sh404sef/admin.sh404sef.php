 ...
 * @version     $Id: admin.sh404sef.php 1213 2010-04-07 09:17:01Z silianacom-svn $
 */
...
function viewSEF($option, $ViewModeId = 0)
{
  ...
  $displayDuplicate = intval($mainframe->getUserStateFromRequest( "viewmode{$option}displayDuplicate", 'display_duplicate', 1));
  ...
  // display duplicates ?
  if (!$displayDuplicate) {
    $where .= empty( $where) ? '': ' AND ' . '`rank` = 0';
  }

  // build show children check box
  if ($ViewModeId == 0) {
  $displayDuplicateList[]     = JHTML::_('select.option', '0', _COM_SEF_DISPLAY_DUPLICATE_URLS_NOT, 'display_duplicate_id', 'display_duplicate_name');
  $displayDuplicateList[]     = JHTML::_('select.option', '1', _COM_SEF_DISPLAY_DUPLICATE_URLS, 'display_duplicate_id', 'display_duplicate_name' );
  $lists['display_duplicate'] = JHTML::_('select.genericlist', $displayDuplicateList, 'display_duplicate', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"','display_duplicate_id', 'display_duplicate_name', $displayDuplicate );
  } else {
    $lists['display_duplicate'] = array();
  }
...
function showConfig($option)
{
  ...
  $lists['joomla_live_site'] = $goodLiveSite ? $lists['joomla_live_site'] :_COM_SEF_TT_JOOMLA_LIVE_SITE_MISSING;
