		...
		switch( $do )
		{
			default:
				...
				if( !$this->attachments_is_writable() && $this->get_total_attachments() > 0 ) {
					printRow( 'Please make sure this ' . JPATH_SITE . $mtconf->get('relative_path_to_attachments') . ' directory is writable. This upgrade has been aborted. Refresh this page once you have made the directory writable.', 0 );
					$this->updated = false;
					return true;
				}
...
ADD `zoom` TINYINT UNSIGNED NOT NULL COMMENT 'Map''s zoom level';");
	...
	function attachments_is_writable() {
		global $mtconf;
		if( is_writable( JPATH_SITE . $mtconf->get('relative_path_to_attachments') ) ) {
			return true;
		} else {
			return false;
		}
