...
DEFINE ('_UDDEIM_HELP_USERLISTS', '<b>Kişiler</b> kişi listesi oluşturmanıza imkan verir (also known as distribution lists). These lists allow to send PMs to multiple recipients. Instead of adding multiple recipients, you can simply enter <i>#listname</i>.');
...
DEFINE ('_UDDEADM_TIMEDELAY_EXP', 'Bir kullanıcının sonraki mesajını göndermeden önce beklemesi gereken saniye cinsinden zaman (0 for no time delay).');
...
DEFINE ('_UDDEADM_BLOCKGROUPS_EXP', 'Kayıtlı kullnacıların mesaj gönderemeyeceği gruplar. Bu ayar sadece kayıtlı kulanıcılar içindir. Özel kullanıcılar ve yöneticiler bu ayardan etkilenmezler. This setting is independent from the individual blocking each user can configure when enabled (see settings above).');
...
DEFINE ('_UDDEADM_HIDEALLUSERS_HEAD', '"Tüm kullanıcılar" listesinde gizle');
...
DEFINE ('_UDDEIM_DELETEDUSER', 'Kullanıcı silindi');
...
DEFINE ('_UDDEADM_CAPTCHAF1', 'sadece genel kullanıcılar');
DEFINE ('_UDDEADM_CAPTCHAF2', 'genel ve kaytlı kullanıcılar');
DEFINE ('_UDDEADM_CAPTCHAF3', 'genel, kayıtlı, özel kullanıcılar');
...
DEFINE ('_UDDEADM_MAINTENANCE_NOTHINGTODO', '<b>Yapılacak birşey yok<b><br>');
...
DEFINE ('_UDDEADM_PMSNOTFOUND', 'Başka bir PMS kurulu değil. Alım mümkün değil.');
...
DEFINE ('_UDDEADM_SYSM_USERNAME_EXP', 'uddeIM sistem mesajlarını destekler. Sistem mesajlarının göndericileri görünmez ve kullanıcılar cevaplayamazlar. Buraya sistem mesajları için varsayılan kullanıcının takma adını girin. (örnek <b>Destek</b> veya <b>Yardım Masası</b> veya <b>Topluluk Yöneticisi</b>)');
...
DEFINE ('_UDDEADM_EMN_BODY_NOMESSAGE_EXP', 'İçerikle ilgili kullanıcının alacağı e-posta ayarı yukarıdadır. İçerik e-posta içerisinde olmayacaktır. Alınacak değişkenler %you%, %user% ve %site% olacaktır. ');		
DEFINE ('_UDDEADM_EMN_BODY_WITHMESSAGE_EXP', 'İçerikle ilgili kullanıcının alacağı e-posta ayarı yukarıdadır. Bu e-posta mesaj mesajı da içerisinde taşıyacaktır. Alınacak değişkenler %you%, %user%, %pmessage% ve %site% olacaktır. ');		
DEFINE ('_UDDEADM_EMN_FORGETMENOT_EXP', 'Kullanıcının alacağı hatırlatma e-postası ile ilgili ayarlar yukarıdadır. Alınacak değişkenler %you% ve %site% olacaktır. ');		
DEFINE ('_UDDEADM_ENABLEDOWNLOAD_EXP', 'İzin verilirse kullanıcılar mesajlarını arşivlerinden indirerek, kendilerine e-posta olarak gönderebilirler.');
...
DEFINE ('_UDDEADM_STOPALLEMAIL_EXP', 'Uddeim in mail göndermesini engellemek için evet seçin (e-posta bildirimleri ve beni unutma e-postaları) irrespective of the users\' settings, for example when testing the site. If you do not want these features ever, set all options above to <i>no</i>.');
...
DEFINE ('_UDDEIM_EMN_EXP', 'Yeni bir özel mesaj aldığınızda e-postanıza bilgi gelir.');
DEFINE ('_UDDEIM_EMN_ALWAYS', 'Yeni mesaj geldiğinde e-posta gönder');
DEFINE ('_UDDEIM_EMN_NONE', 'Yeni mesaj geldiğinde e-posta gönderme');
DEFINE ('_UDDEIM_EMN_WHENOFFLINE', 'Çevrimdışıyken mesaj gelirse e-posta gönder');
DEFINE ('_UDDEIM_EMN_NOTONREPLY', 'Cevap geldiğinde e-posta yollama');
