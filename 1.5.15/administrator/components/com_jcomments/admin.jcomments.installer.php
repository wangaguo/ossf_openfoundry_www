...
class JCommentsInstaller
{
	function postInstall()
	{
		...
		if (empty($version)) {
			...
			$db->setQuery("INSERT IGNORE INTO `#__jcomments_version` (`version`,`installed`) VALUES ('$version', '$currentDate')");
			@$db->query();
		} else {
			$currentDate = date('Y-m-d H:i:s');
			$db->setQuery("UPDATE `#__jcomments_version` SET `version` = '$version', `updated` = '$currentDate';");
			@$db->query();
		}
		...
		if (JCOMMENTS_JVERSION == '1.0') {
			...
			if (is_writable($menuiconpath)) {
				...
				$res7 = @copy($currentIconsPath . DS . 'custombbcode16x16.png', $menuiconpath . DS . 'custombbcode16x16.png');
				ob_end_clean();
			...
			@$db->query();
			$db->setQuery("UPDATE #__components SET admin_menu_img='$adminIconsPath/custombbcode16x16.png', name='" . JText::_('Custom BBCode') . "'" . "\n WHERE admin_menu_link='option=com_jcomments&task=custombbcodes'");
			@$db->query();
			...
			@$db->query();
			$db->setQuery("UPDATE #__components SET admin_menu_img='class:jcomments-subscriptions' WHERE admin_menu_link='option=com_jcomments&task=subscriptions'");
			@$db->query();
			$db->setQuery("UPDATE #__components SET admin_menu_img='class:jcomments-custombbcode' WHERE admin_menu_link='option=com_jcomments&task=custombbcodes'");
			@$db->query();
		...
		// there are no changes in database structure since 2.0.0.x
		$dbStructureChanged = (version_compare($version, '2.0.0.0') < 0);
		if ($dbStructureChanged) {
			// auto upgrade old table structure
			require_once (dirname(__FILE__) . DS . 'install' . DS . 'helpers' . DS . 'database.php');
			$upgradeStructure = JCommentsInstallerDatabaseHelper::upgradeStructure();
			if ($upgradeStructure) {
...
<table width="95%" border="0" cellpadding="0" cellspacing="0">
	<tr valign="top" align="left">
		<td width="50px"><img src="<?php echo $mainframe->getCfg( 'live_site' ); ?>/administrator/components/com_jcomments/images/jcomments48x48.png" border="0" alt="" /></td>
...
<tr valign="top" align="right">
		...
		<div class="next"><a href="<?php echo $mainframe->getCfg( 'live_site' ); ?>/administrator/<?php echo JCOMMENTS_INDEX; ?>?option=com_jcomments&task=settings"><?php echo JText::_('AI_NEXT'); ?></a>
		</div>
		</div>
		</td>
</tr>
