<?php
/**
 * @version		$Id: mtree.php 582 2009-03-12 03:58:15Z CY $
 * @package		Mosets Tree
...
function plgSearchMtree( $text, $phrase='', $ordering='', $areas=null )
{
	...
	if( $search_listing && !empty($where) && $limit > 0 ) {
		...
		$sql .= ' LEFT JOIN #__mt_cats AS c ON c.cat_id = cl.cat_id ' 
			. ' WHERE '
			. " link_published='1' AND link_approved='1' AND ( publish_up = '0000-00-00 00:00:00' OR publish_up <= NOW()  ) "
			. ' AND ( publish_down = \'0000-00-00 00:00:00\' OR publish_down >= NOW() )'
			. ' AND cl.link_id = l.link_id '
			. ' AND cl.main = 1 ';
		$sql .= ' AND '.$where
			.	' ORDER BY ' 
			. $mtconf->get('first_search_order1') . ' ' . $mtconf->get('first_search_order2') 
			. ', ' . $mtconf->get('second_search_order1') . ' ' . $mtconf->get('second_search_order2');
