 ...
 * @version     $Id: sh404sef.inc.php 1350 2010-05-04 17:01:31Z silianacom-svn $
 *
 */
...
switch ($shPageInfo->URI->path) {
    ...
    if (is_null($option)) {
      ...
      || ( (count($_GET)+count($_POST)) == 1 && !empty($lang))){
        ...
        if (strpos( $_SERVER['SERVER_SOFTWARE'], 'Microsoft-IIS') !== false) { // IIS adds index.php to $_SERVER['REQUEST_URI'], even if it
          ...
          _log( 'IIS not detected, $current calculated as : ' . $current);
        }
      ...
      if (strtolower( $shPageInfo->index) == 'index.php') {  // prevents loading homepage if accessing index2.php (ajax calls)
  ...
  default: {
    ...
    if (is_valid($shPageInfo->QUERY_STRING)) {
      ...
      // check for aliases !! duplicated code @todo : remove
      $incomingUrl = $sefConfig->shEncodeUrl ? shUrlEncode( $path) :  $path; // fix for encoded urls, thanks ByeVas
