...
class JComments
{
	...
	function sendNotification( &$comment, $isNew = true)
	{
		...
		if ($config->get('notification_email') != '') {
			...
			if ($isNew) {
				$subject = '[Manage]' . JText::sprintf('NOTIFICATION_SUBJECT_NEW', $object->object_title);
			} else {
				$subject = '[Manage]' . JText::sprintf('NOTIFICATION_SUBJECT_UPDATED', $object->object_title);
			}
...
// 
	...
	function sendNotificationToCategoryMonitors( &$comment, $isNew = true)
	{
		global $mainframe, $my;
		$config = & JCommentsFactory::getConfig();
		if ($config->get('notification_emails_of_categories') != '') {
            $dbo = & JCommentsFactory::getDBO();
            // get article's catid
            $query = "SELECT catid from #__content as c LEFT JOIN #__jcomments as j on j.object_id = c.id WHERE j.object_id=".$comment->object_id." LIMIT 0,1";
            $dbo->setQuery( $query );
            $rows = $dbo->loadObjectList();
            ...
            if(isset($rows[0]->catid) && $arrCat[$rows[0]->catid] != '') {
                $object = new stdClass();
                $object->object_title = JCommentsObjectHelper::getTitle($comment->object_id, $comment->object_group);
                $object->object_link = JCommentsObjectHelper::getLink($comment->object_id, $comment->object_group);
                $object->object_link = JCommentsFactory::getAbsLink($object->object_link);
                ...
                if ($isNew) {
                    $subject = '[Catalogue]' . JText::sprintf('NOTIFICATION_SUBJECT_NEW', $object->object_title);
                } else {
                    $subject = '[Catalogue]' . JText::sprintf('NOTIFICATION_SUBJECT_UPDATED', $object->object_title);
                }

                if (isset($subject) && isset($message)) {
                    $emails = explode(',',  $arrCat[$rows[0]->catid]);
	...
	function sendToAuthors( &$comment, $isNew = true)
	{
		global $mainframe;
		$dbo = & JCommentsFactory::getDBO();
		$config = & JCommentsFactory::getConfig();

        // get author's id from
        $auids=explode(',',$config->get('disabled_notification_authors'));
        $query = "SELECT distinct created_by as authorId, u.email from #__content c LEFT JOIN #__jcomments cm ON c.id=cm.object_id LEFT JOIN #__users u on c.created_by=u.id where cm.object_id=".$comment->object_id;
        $dbo->setQuery( $query); 
        $rows = $dbo->loadObjectList();
        if(count($rows[0])>0) {
            $authorId = $rows[0]->authorId;
        }
        if(!in_array($authorId, $auids)) {
            // send email to article's author
			$object_title = JCommentsObjectHelper::getTitle($comment->object_id, $comment->object_group);
			$object_link = JCommentsObjectHelper::getLink($comment->object_id, $comment->object_group);
			$object_link = JCommentsFactory::getAbsLink($object_link);
			...
			if ($isNew) {
				$subject = '[Author]' . JText::sprintf('NOTIFICATION_SUBJECT_NEW', $object_title);
			} else {
				$subject = '[Author]' . JText::sprintf('NOTIFICATION_SUBJECT_UPDATED', $object_title);
			}
			...
			if (isset($subject)) {
				...
				foreach ($rows as $row) {
					...
					JCommentsMail::send($mailFrom, $fromName, $rows[0]->email, $subject, $message, true);
				}
			}
