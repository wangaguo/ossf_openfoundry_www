...
function createImg($link = "1", $url, $or = "width", $size = "100%", $sfx ="", $path, $class, $imgpar) {
	...
	$imgurl = xtCompat::sefRelToAbs($path.$url);
	$html = "";
...
//	$html .= ">";
		if ($this->type == "select") {
			...
			if (!empty ($options[0]['img_url'])) {
			 	$html = createImg($options[0]['imglink'], $options[0]['img_url'], $options[0]['imgor'], $options[0]['imgsize'], $this->sfx, "", $tabclass_arr[$tabcnt], $this->imgpar);
			}
            ...
            foreach ($options as $o) { 
				...
				if (!empty($o['linkurl'])) {
					if (!empty($o['linktext'])) $t = $o['linktext'];
					else $t = $o['linkurl'];
					
					$alink[$i] = "<a target=\"_blank\" href=\"".$o['linkurl']."\">".$o['linktext']."</a>";
				}
				
				if (!empty ($o['img_url'])) {
			 	$img[$i] = createImg($o['imglink'], $o['img_url'], $o['imgor'], $o['imgsize'], $this->sfx, "", $tabclass_arr[$tabcnt], $this->imgpar);
				}
				$lab = "";
				...
				$o_rest = mb_substr($o[2],mb_strlen($o_intro), mb_strlen($o[2]),'UTF-8');
				
				$inp[$i]  = "<div class='".$tabclass_arr[$tabcnt]."'>";
	       	 	if ($o['desc'] && $this->conf->tooltips) 
					$inp[$i] .= "<span class=\"tipxt\" title=\"".$o[2]."::".$o['desc']."\">";
				...
				if ($alink[$i] !="") $html .= "<div id=\"pollxtLinkCol\">".$alink[$i]."</div>";
