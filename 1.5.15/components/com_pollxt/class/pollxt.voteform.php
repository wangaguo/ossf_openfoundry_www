...
//		$frontend->func="document.getElementById('captcha').style.display='block'";
...
//			$frontend->func = "document.getElementById('poll".$this->data->name."').style.display=\"none\";";
	...
	function getIntro() {
		...
		if (!empty ($poll->img_url)) {
			$pollimg = createImg($poll->imglink, $poll->img_url, $poll->imgor, 
			$poll->imgsize, $this->conf->sfx, "", 
			"sectiontableheader", $this->conf->imgpar);
		}
		...
		if ($poll->intro) { 
  			$html .= "<div id=\"pollxtIntro\">".str_replace("\n", "<br/>", charEnc($poll->intro))."</div>";
		} 
		return $html;
	}
	function getOutro() {
...
//				echo $q[3];			 
			elseif ($q[3] == "6" ) {
					...
					$this->htmlQuestion[$qc]  = "<div class=\"sectiontableheader".$sfx."\">";
		       	 	if ($q['desc'] && $this->conf->tooltips) 
						$this->htmlQuestion[$qc] .= "<span class=\"tipxt\" title=\"".$q[2]."::".$q['desc']."\">";
       				if ($this->conf->hide != "2")
					...
					if (!empty($q['img_url'])) { 
					$this->htmlQuestion[$qc]  .= createImg($q['imglink'], $q['img_url'], $q['imgor'], $q['imgsize'], $sfx, "", "sectiontableheader", $imgpar);
					}
		...
		$database = JFactory::getDBO();			
			...
			for ($i = 0, $n = count($questions); $i < $n; $i ++) {
				...
				foreach ($o as $p) {
					...
					$options[$i][$j]['desc'] = $p->desc;
					$options[$i][$j]['linkurl'] = $p->linkurl;
					$options[$i][$j]['linktext'] = $p->linktext;
