...
class sef_mtree {
	...
	function create ($string) {
		...
		if ( eregi("&amp;task=listcats&amp;cat_id=",$string) ) {
			...
			if( $this->getLimits( $limit, $limitstart, $string ) ) {
				$page = (($limitstart / $mtconf->get('fe_num_of_links')) +1);
		...
		# My Page
		if ( $this->taskIs("mypage", $string) ) {

			$sefstring .= _MT_SEF_MYPAGE .'/';
		...
		if ( $this->taskIs("listnew", $string) ) {
			...
			if( $this->getLimits( $limit, $limitstart, $string ) ) {
				$page = (($limitstart / $mtconf->get('fe_num_of_newlisting')) +1);
				$sefstring .= $page . '/';
			} else {
				$sefstring .= '1' . '/';
			}
		...
		if ( $this->taskIs("listupdated", $string) ) {
			$cat_id = $this->getID( 'cat', $string );
			$sefstring .= $this->appendCat( $cat_id );
			$sefstring .= _MT_SEF_RECENTLYUPDATED . '/';
		}

		# Most Favoured Listing
		if ( $this->taskIs("listfavourite", $string) ) {
			$cat_id = $this->getID( 'cat', $string );
			$sefstring .= $this->appendCat( $cat_id );
			$sefstring .= _MT_SEF_MOSTFAVOURED . '/';
		}
		...
		if ( $this->taskIs("listalpha", $string) ) {
			...
			$temp = split("&", $temp[1]);
			$sefstring .= urlencode($temp[0]) . '/';

			if( $this->getLimits( $limit, $limitstart, $string ) ) {
				//	http://example.com/c/mtree/Computer/Games/List_Alpha/d/page25.html
		...
		if ( $this->taskIs("advsearch2", $string) ) {
			...
			if( $this->getLimits( $limit, $limitstart, $string ) ) {
				$page = (($limitstart / $mtconf->get('fe_num_of_searchresults')) +1);
			}
		...
		if ( $this->taskIs("viewowner", $string) || $this->taskIs("viewusersreview", $string) || $this->taskIs("viewusersfav", $string) ) {

			$user_id = $this->getID( 'user', $string );
			$database->setQuery( "SELECT username FROM #__users WHERE id='".$user_id."' AND block='0'" );
			$username = $database->loadResult();

			if ( !empty($username) ) {
				if( $this->taskIs("viewowner", $string) ) {
					$sefstring .= _MT_SEF_OWNER;
				} elseif( $this->taskIs("viewusersreview", $string) ) {
					$sefstring .= _MT_SEF_REVIEWS;
				} else {
					$sefstring .= _MT_SEF_FAVOURITES;
				}
				$sefstring .= '/' . sefencode($username) . '/';
			}
		...
		# View Image
		if ( $this->taskIs("viewimage",$string) ) {
			$sefstring .= _MT_SEF_IMAGE . '/' . $this->getID('img',$string) . '/';
		}
		...
		if ( $this->listingTaskIs("viewlink",$string) ) {
			...
			if( $this->getLimits( $limit, $limitstart, $string ) ) {
				...
				$page = (($limitstart / $mtconf->get('fe_num_of_reviews')) +1);
		...
		# View Gallery
		if ( $this->listingTaskIs("viewgallery",$string) ) {
			$mtLink = new mtLinks( $database );
			$mtLink->load( $this->getID('link',$string) );
			$sefstring .= $this->appendCatListing( $mtLink, false );
			$sefstring .= _MT_SEF_GALLERY . '/';
		}
		
		# Reply Review
		if ( $this->taskIs("replyreview", $string) ) {
			$sefstring .= _MT_SEF_REPLYREVIEW . '/' . $this->getID( 'rev', $string ) . '/';
		}

		# Report Review
		if ( $this->taskIs("reportreview", $string) ) {
			$sefstring .= _MT_SEF_REPORTREVIEW . '/' . $this->getID( 'rev', $string ) . '/';
		}
		...
		# DELETE LISTING
		if ( $this->listingTaskIs("deletelisting",$string) ) {
			$mtLink = new mtLinks( $database );
			$mtLink->load( $this->getID('link',$string) );
			$sefstring .= $this->appendCatListing( $mtLink, false );
			$sefstring .= _MT_SEF_DELETE . '/';
		}
	...
	* Output: $QUERY_STRING, string, Joomla! query string (var1=$var1&var2=$var2)
	...
	function revert ($url_array, $pos) {
		...
		if ( (isset($url_array[$pos+2]) && $url_array[$pos+2]!="") ) {
			...
			if ( 
						...
						&&
							( !in_array( $url_array[ (count($url_array)-3) ], array( _MT_SEF_FEATUREDLISTING, _MT_SEF_NEWLISTING, _MT_SEF_REPLYREVIEW, _MT_SEF_REPORTREVIEW, _MT_SEF_IMAGE ) ) )
						&&
							( !in_array( $url_array[ (count($url_array)-4) ], array( _MT_SEF_OWNER, _MT_SEF_REVIEWS, _MT_SEF_FAVOURITES, _MT_SEF_SEARCH, _MT_SEF_ADVSEARCH2, _MT_SEF_LISTALPHA ) ) )
						&&
							!in_array( $url_array[ (count($url_array)-2) ], array( _MT_SEF_RECENTLYUPDATED, _MT_SEF_MOSTFAVOURED, _MT_SEF_POPULARLISTING, _MT_SEF_MOSTRATEDLISTING, _MT_SEF_TOPRATEDLISTING, _MT_SEF_MOSTREVIEWEDLISTING, _MT_SEF_REVIEW, _MT_SEF_RATE, _MT_SEF_RECOMMEND, _MT_SEF_CONTACT, _MT_SEF_REPORT, _MT_SEF_CLAIM, _MT_SEF_VISIT, _MT_SEF_DELETE, _MT_SEF_ADDLISTING, _MT_SEF_ADDCATEGORY, _MT_SEF_REVIEWS_PAGE, _MT_SEF_DETAILS, _MT_SEF_GALLERY ) )
						&&
				...
				switch( end($cat_names) ) {
					...
					case _MT_SEF_MYPAGE:
						$QUERY_STRING .= $this->appendURL( 'task', 'mypage' );
						$pagenumber = $this->getPageNumber($url_array);
						
						if ( $pagenumber > 0 ) {
							$QUERY_STRING .= $this->appendURL( 'limit', $mtconf->get('fe_num_of_links') );
							$QUERY_STRING .= $this->appendURL( 'limitstart', ($mtconf->get('fe_num_of_links') * ($pagenumber -1)) );
						}
						break;
			...
			//} elseif( $url_array[ (count($url_array)-2) ] == _MT_SEF_FEATUREDLISTING ) {
			} elseif( in_array( $url_array[ (count($url_array)-2) ], array( _MT_SEF_RECENTLYUPDATED, _MT_SEF_MOSTFAVOURED, _MT_SEF_POPULARLISTING, _MT_SEF_MOSTRATEDLISTING, _MT_SEF_TOPRATEDLISTING, _MT_SEF_MOSTREVIEWEDLISTING, _MT_SEF_REVIEW, _MT_SEF_RATE, _MT_SEF_RECOMMEND, _MT_SEF_CONTACT, _MT_SEF_REPORT, _MT_SEF_CLAIM, _MT_SEF_VISIT, _MT_SEF_DELETE, _MT_SEF_ADDLISTING, _MT_SEF_ADDCATEGORY, _MT_SEF_REVIEWS_PAGE, _MT_SEF_DETAILS, _MT_SEF_GALLERY ) ) || eregi(_MT_SEF_REVIEWS_PAGE."[0-9]+",$url_array[ (count($url_array)-2) ]) ) {
				...
				switch($vfilename) {
					...
					case _MT_SEF_RECENTLYUPDATED:
						...
						$cat_id = $this->findCatID( $cat_names );
						$QUERY_STRING .= $this->appendURL( 'task', 'listupdated' );
						$QUERY_STRING .= $this->appendURL( 'cat_id', $cat_id );
						break;
					...
					case _MT_SEF_MOSTFAVOURED:
						$cat_names = array_slice( $url_array, ($pos+2), -2 );
						$cat_id = $this->findCatID( $cat_names );
						$QUERY_STRING .= $this->appendURL( 'task', 'listfavourite' );
						$QUERY_STRING .= $this->appendURL( 'cat_id', $cat_id );
						break;
					...
					case _MT_SEF_DELETE:
						$path_names = array_slice( $url_array, ($pos+2), -2 );
						$link_id = $this->findLinkID( $path_names );
						$QUERY_STRING .= $this->appendURL( 'task', 'deletelisting' );
						$QUERY_STRING .= $this->appendURL( 'link_id', $link_id );
						break;
					...
					case _MT_SEF_GALLERY:
						$path_names = array_slice( $url_array, ($pos+2), -2 );
						$link_id = $this->findLinkID( $path_names );
						$QUERY_STRING .= $this->appendURL( 'task', 'viewgallery' );
						$QUERY_STRING .= $this->appendURL( 'link_id', $link_id );
						break;
					...
					default:
						...
						if ( $isReviewsPage ) {
							...
							$QUERY_STRING .= $this->appendURL( 'limit', $mtconf->get('fe_num_of_reviews') );
							$QUERY_STRING .= $this->appendURL( 'limitstart', ($mtconf->get('fe_num_of_reviews') * ($pagenumber -1)) );
			...
			#   - http://example.com/c/mtree/Game/Action/List_Alpha/[a-z]/[0-9]+
				...
				$alpha = $url_array[ (count($url_array)-3) ];
				...
				$QUERY_STRING .= $this->appendURL( 'alpha', $alpha );
				$QUERY_STRING .= $this->appendURL( 'limit', $mtconf->get('fe_num_of_links') );
				$QUERY_STRING .= $this->appendURL( 'limitstart', ($mtconf->get('fe_num_of_links') * ($page -1)) );
			...
			#   - http://example.com/c/mtree/Game/Action/New_Listing/[0-9]+
				...
				$QUERY_STRING .= $this->appendURL( 'limit', $mtconf->get('fe_num_of_newlisting') );
				$QUERY_STRING .= $this->appendURL( 'limitstart', ($mtconf->get('fe_num_of_newlisting') * ($page -1)) );

			# (6) List Owner's listings
			#   - http://example.com/c/mtree/viewowner/Gandalf/1/  _MT_SEF_REVIEWS, _MT_SEF_FAVOURITES,
			} elseif( $url_array[ (count($url_array)-4) ] == _MT_SEF_OWNER || $url_array[ (count($url_array)-4) ] == _MT_SEF_REVIEWS || $url_array[ (count($url_array)-4) ] == _MT_SEF_FAVOURITES ) {
				
				switch( $url_array[ (count($url_array)-4) ] ) {
					case _MT_SEF_OWNER:
						$QUERY_STRING .= $this->appendURL( 'task', 'viewowner' );
						break;
					case _MT_SEF_REVIEWS:
						$QUERY_STRING .= $this->appendURL( 'task', 'viewusersreview' );
						break;
					case _MT_SEF_FAVOURITES:
						$QUERY_STRING .= $this->appendURL( 'task', 'viewusersfav' );
						break;
				}

				$owner_username = $url_array[ (count($url_array)-3) ];
				$owner_username = sefdecode($owner_username);

				$database->setQuery( "SELECT id FROM #__users WHERE username='".$owner_username."' LIMIT 1" );
				$user_id = $database->loadResult();
				$QUERY_STRING .= $this->appendURL( 'user_id', $user_id );

				// Get the page number
				$pagenumber = $url_array[ (count($url_array)-2) ];
			...
			# (7) Reply Review
			#   - http://example.com/c/mtree/replyreview/123/
			} elseif( $url_array[ (count($url_array)-3) ] == _MT_SEF_REPLYREVIEW ) {

				$rev_id = $url_array[ (count($url_array)-2) ];
				$QUERY_STRING .= $this->appendURL( 'task', 'replyreview' );
				$QUERY_STRING .= $this->appendURL( 'rev_id', $rev_id );

			# (8) Report Review
			#   - http://example.com/c/mtree/reportreview/123/
			} elseif( $url_array[ (count($url_array)-3) ] == _MT_SEF_REPORTREVIEW ) {

				$rev_id = $url_array[ (count($url_array)-2) ];
				$QUERY_STRING .= $this->appendURL( 'task', 'reportreview' );
				$QUERY_STRING .= $this->appendURL( 'rev_id', $rev_id );
			...
			#		- http://example.com/c/mtree/Search/1/searchword/
				...
				$QUERY_STRING .= $this->appendURL( 'limit', $mtconf->get('fe_num_of_searchresults') );
				$QUERY_STRING .= $this->appendURL( 'limitstart', ($mtconf->get('fe_num_of_searchresults') * ($page -1)) );

			# (10) Advanced Search Result
			...
			# (11) View Image
			#		- http://example.com/c/mtree/image/[img_id]
			} elseif( $url_array[ (count($url_array)-3) ] == _MT_SEF_IMAGE ) {

				$img_id = $url_array[ (count($url_array)-2) ];
				$QUERY_STRING .= $this->appendURL( 'task', 'viewimage' );
				$QUERY_STRING .= $this->appendURL( 'img_id', $img_id );
