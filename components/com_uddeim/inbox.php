...
function uddeIMshowInbox($myself, $item_id, $limit, $limitstart, $cryptpass, $config, $filter_user, $filter_unread) {
	...
	$allmessages = uddeIMselectInbox($myself, $limitstart, $limit, $config, $filter_user, $filter_unread);
...
//	}
	...
	$referlink = "index.php?option=com_uddeim&task=inbox&Itemid=".$item_id.$addlink;
	if ($totalinbox>$limit) {
		...
		echo "[<a href='".uddeIMsefRelToAbs("index.php?option=com_uddeim&task=inbox&Itemid=".$item_id."&limitstart=0&limit=".$totalinbox.$addlink)."'>"._UDDEIM_SHOWALL."</a>]";
		echo "</div>\n";
	}
...
function uddeIMshowMessage($myself, $item_id, $messageid, $isforward, $cryptpass, $config) {
	...
	foreach($displaymessages as $displaymessage) {
		...
		$headerstring.="</tr>";
		$msgnavigation = "&nbsp;";
		$headerstring.="<tr><td valign='buttom'><div class='uddeim-messagefrom'>".$msgnavigation."</div></td></tr>";
		$headerstring.="</table>";
