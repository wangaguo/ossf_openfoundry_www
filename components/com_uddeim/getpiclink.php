...
function uddeIMgetLinkOnly($ofanid, $ofaname, $config) {	// LINK only
	...
	if ($config->showcblink==1) {			// CB
		...
		$gimmeback = "<a href='".uddeIMsefRelToAbs("index.php?option=com_cbe&task=userProfile&user=".(int)$ofanid)."'>".$ofaname."</a>";
	} else {
		$gimmeback = $ofaname;
	}
	return $gimmeback;
...
//		}
		...
		$filenamelocal  = uddeIMgetPath('absolute_path')."/images/cbe/tn".$ofanavatar;		// Thumbnail
		$filenamelive   = uddeIMgetPath('live_site')    ."/images/cbe/tn".$ofanavatar;		// Thumbnail

		$filenameglocal = uddeIMgetPath('absolute_path')."/images/cbe/".$ofanavatar;		// Gallery
		$filenameglive  = uddeIMgetPath('live_site')    ."/images/cbe/".$ofanavatar;		// Gallery

		// NOPHOTO for CBE new
		$filename3local = uddeIMgetPath('absolute_path')."/components/com_cbe/images/".$fileLang."/tnnophoto.jpg";
...
function uddeIMgetStyleForThumb($config) {
	...
	if ($config->getpiclink && $config->showcbpic==1) {
		...
		} elseif (file_exists(uddeIMgetPath('absolute_path')."/administrator/components/com_cbe/ue_config.php")) {
