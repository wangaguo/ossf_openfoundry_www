...
$autorespondercheck = uddeIMmosGetParam ($_POST, 'autorespondercheck');		// "on"
$autorespondertext  = uddeIMmosGetParam ($_POST, 'autorespondertext');
$autoforwardcheck   = uddeIMmosGetParam ($_POST, 'autoforwardcheck');		// "on"
...
// --------------------------------------------------
	...
	case "inbox":
		...
		uddeIMshowInbox($userid, $item_id, $limit, $limitstart, $cryptpass, $config, $filter_user, $filter_unread);
		break;
...
// --------------------------------------------------
	...
	case "outbox":
		...
		uddeIMshowOutbox($userid, $item_id, $limit, $limitstart, $cryptpass, $config, $filter_user, $filter_unread);
		break;
...
// --------------------------------------------------
	...
	case "archive":
		...
		uddeIMarchive($userid, $item_id, $limit, $limitstart, $cryptpass, $config, $filter_user, $filter_unread);
		break;
...
//				$to_name=stripslashes($to_name_bak);		// all names
	...
	foreach ($anames as $value) {
		...
		if ($config->cryptmode==1) {
			...
			$sql  = "INSERT INTO #__uddeim (fromid, toid, replyid, message, datum, cryptmode, crypthash) VALUES (".(int)$savefromid.", ".(int)$savetoid.", ".(int)$replyid.", '".$cm."', ".$savedatum.",1,'".md5($config->cryptkey)."')";
			...
			$sql  = "INSERT INTO #__uddeim (fromid, toid, replyid, message, datum, cryptmode, crypthash) VALUES (".(int)$savefromid.", ".(int)$savetoid.", ".(int)$replyid.", '".$cm."', ".$savedatum.",".$themode.",'".md5($thepass)."')";
		} elseif ($config->cryptmode==3) {
			$themode = 3;
			$cm = uddeIMencrypt($savemessage,"",CRYPT_MODE_STOREBASE64);
			$sql  = "INSERT INTO #__uddeim (fromid, toid, replyid, message, datum, cryptmode) VALUES (".(int)$savefromid.", ".(int)$savetoid.", ".(int)$replyid.", '".$cm."', ".$savedatum.",3)";
		} else {
			$sql  = "INSERT INTO #__uddeim (fromid, toid, replyid, message, datum) VALUES (".(int)$savefromid.", ".(int)$savetoid.", ".(int)$replyid.", '".$savemessage."', ".$savedatum.")";
		}
		...
		// autoforward code
		// ##################################################################################################
		if ($config->autoforward==1 || ($config->autoforward==2 && ($rec_gid==24 || $rec_gid==25))) {
...
[i]("._UDDEIM_THISISACOPY.$to_name.")[/i]";
			...
			if ($config->cryptmode==1) {
				$cm = uddeIMencrypt($savemessagecopy,$config->cryptkey,CRYPT_MODE_BASE64);
				$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox, cryptmode, crypthash) VALUES (".(int)$savefromid.", ".(int)$savefromid.", ".(int)$replyid.", 1, '".$cm."', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).",1,'".md5($config->cryptkey)."')";
			} elseif ($config->cryptmode==2) {
				$themode=2;
				$thepass=$cryptpass;
				if (!$thepass) {	// no password entered, then fallback to obfuscating
				...
				$cm = uddeIMencrypt($savemessagecopy,$thepass,CRYPT_MODE_BASE64);
				$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox, cryptmode, crypthash) VALUES (".(int)$savefromid.", ".(int)$savefromid.", ".(int)$replyid.", 1, '".$cm.             "', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).",".$themode.",'".md5($thepass)."')";
			} elseif ($config->cryptmode==3) {
				$cm = uddeIMencrypt($savemessagecopy,"",CRYPT_MODE_STOREBASE64);
				$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox, cryptmode) VALUES (".(int)$savefromid.", ".(int)$savefromid.", ".(int)$replyid.", 1, '".$cm."', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).",3)";
			} else {
				$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox) VALUES (".         				(int)$savefromid.", ".(int)$savefromid.", ".(int)$replyid.", 1, '".$savemessagecopy."', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).")";
			}
			$database->setQuery($sql);
			if (!$database->query()) {
				die("SQL error when attempting to save a message" . $database->stderr(true));
		...
		// autoresponder
		// ##################################################################################################
		if ($config->autoresponder==1 || ($config->autoresponder==2 && ($rec_gid==24 || $rec_gid==25))) {
			$ison = uddeIMgetEMNautoresponder($savetoid);
			if ($ison==1) {

// BUGBUG: An autoresponder message is created and the outbox message is marked deleted.
// This is not a bug since in my opinion it does not make sense to store autoresponder messages AND the received message.
				...
				$sql="INSERT INTO #__uddeim (fromid, toid, message, datum, totrashoutbox, totrashdateoutbox) VALUES (".(int)$savetoid.", ".(int)$savefromid.", '". addslashes(strip_tags($autorespondertext)) ."', ".$savedatum.", 1,".uddetime($config->timezone).")";
				// $sql="INSERT INTO #__uddeim (fromid, toid, message, datum) VALUES (".(int)$savetoid.", ".(int)$savefromid.", '". _UDDEIM_AUTORESPONDER_DEFAULT ."', ".$savedatum.")";
				$database->setQuery($sql);
				if (!$database->query()) {
					die("SQL error when attempting to save a message" . $database->stderr(true));
				}
...
[i]("._UDDEIM_THISISACOPY.$var_toname.")[/i]";
		...
		if ($config->cryptmode==1) {
			$cm = uddeIMencrypt($savemessagecopy,$config->cryptkey,CRYPT_MODE_BASE64);
			$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox, cryptmode, crypthash) VALUES (".(int)$myself.", ".(int)$myself.", ".(int)$replyid.", 1, '".$cm."', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).",1,'".md5($config->cryptkey)."')";
		} elseif ($config->cryptmode==2) {
			$themode=2;
			$thepass=$cryptpass;
			if (!$thepass) {	// no password entered, then fallback to obfuscating
			...
			$cm = uddeIMencrypt($savemessagecopy,$thepass,CRYPT_MODE_BASE64);
			$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox, cryptmode, crypthash) VALUES (".(int)$myself.", ".(int)$myself.", ".(int)$replyid.", 1, '".$cm.             "', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).",".$themode.",'".md5($thepass)."')";
		} elseif ($config->cryptmode==3) {
			$cm = uddeIMencrypt($savemessagecopy,"",CRYPT_MODE_STOREBASE64);
			$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox, cryptmode) VALUES (".(int)$myself.", ".(int)$myself.", ".(int)$replyid.", 1, '".$cm."', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).",3)";
		} else {
			$sql="INSERT INTO #__uddeim (fromid, toid, replyid, toread, message, datum, disablereply, systemmessage, archived, totrashoutbox, totrashdateoutbox) VALUES (".         				(int)$myself.", ".(int)$myself.", ".(int)$replyid.", 1, '".$savemessagecopy."', ".$savedatum.", 1, '".$copyname."', 0, 1,".uddetime($config->timezone).")";
		}
		$database->setQuery($sql);
		if (!$database->query()) {
			die("SQL error when attempting to save a message" . $database->stderr(true));
...
function uddeIMsaveSysgm($myself, $to_name, $to_id, $pmessage, $tobedeleted, $item_id, $messageid, $sysgm_sys, $sysgm_universe, $sysgm_validfor, $sysgm_really, $cryptpass, $config) {
	...
	if (!$sysgm_really) {
		...
		foreach($receivers as $receiver) {
			...
			// Check if E-Mail notification or popups are enabled by default, if so create a record for the receiver.
			if ($config->notifydefault>0 || $config->popupdefault>0 || $config->pubfrontenddefault>0 || $config->autoresponder>0) {
				if (!uddeIMexistsEMN($savetoid))
					uddeIMinsertEMNdefaults($savetoid, $config);
			}
			...
			if($config->allowemailnotify==1) {
				...
				if ($ison) {
					...
					if (($ison==1) || ($ison==2 && !$currentlyonline) || ($ison==10 && 	!$itisareply) || ($ison==20 && !$currentlyonline && !$itisareply))  {
						uddeIMdispatchEMN($insID, $item_id, $themode, $emn_fromid, $savetoid, $email, 0, $config);
						// 0 stands for normal (not forgetmenot)
					}
				}
			} elseif($config->allowemailnotify==2) {
...
function uddeIMshowSettings($myself, $item_id, $config) {
	...
	if ($config->autoresponder==1 || ($config->autoresponder==2 && ($my_gid==24 || $my_gid==25))) {
		$emptysettings='';
		...
		if ($config->maxlength>0)		// because if 0 do not use any maxlength
			$autorespondertext = substr($autorespondertext, 0, $config->maxlength);
		echo "<div class='uddeim-set-block'>";  // was uddeim-set-emn
		...
		echo '<input onclick="document.emnrespform.autorespondercheck.checked ? document.emnrespform.autorespondertext.disabled=false : document.emnrespform.autorespondertext.disabled=true;" type="checkbox" '.$emn_responder_checkstatus.' name="autorespondercheck" />'._UDDEIM_EMN_AUTORESPONDER.'<br />';
	...
	if ($config->autoforward==1 || ($config->autoforward==2 && ($my_gid==24 || $my_gid==25))) {
		$emptysettings='';
		...
		echo '<input onclick="document.emnfwdform.autoforwardcheck.checked ? document.emnfwdform.autoforwardid.disabled=false : document.emnfwdform.autoforwardid.disabled=true;" type="checkbox" '.$emn_forward_checkstatus.' name="autoforwardcheck" />'._UDDEIM_EMN_AUTOFORWARD.'<br />';
...
function uddeIMshowAbout($myself, $item_id, $versionstring, $config) {
	...
	echo "&copy; 2007-2009 Stephan Slabihoud, &copy; 2005-2006 by Benjamin Zweifel</p>\n";
