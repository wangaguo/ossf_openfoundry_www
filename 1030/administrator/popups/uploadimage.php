...
/* todosjuntos.org */
$_SESSION['adminlanguage']['iso'] = $mosConfig_lang;
include_once "../translator.php";
$_LANG = new joomla_I18N();
$_LANG->loadAll($option);
...
if (isset($_FILES['userfile'])) {
	...
	if (empty($userfile_name)) {
		mosErrorAlert($_LANG->_( 'Please select an image to upload' ), $action);
	}
	...
	if (eregi('.pdf', $userfile_name) || eregi('.doc', $userfile_name) || eregi('.xls', $userfile_name) || eregi('.ppt', $userfile_name)) {
		if (!move_uploaded_file ($_FILES['userfile']['tmp_name'],$media_path.$_FILES['userfile']['name']) || !mosChmod($media_path.$_FILES['userfile']['name'])) {
			...
			mosErrorAlert($_LANG->_( 'Upload of' ). " $userfile_name ". $_LANG->_( 'to' )." $media_path ". $_LANG->_( 'successful' ), "window.close()");
		}
	} elseif (!move_uploaded_file ($_FILES['userfile']['tmp_name'],$base_Dir.$_FILES['userfile']['name']) || !mosChmod($base_Dir.$_FILES['userfile']['name'])) {
			mosErrorAlert($_LANG->_( 'Upload of' ). " $userfile_name ". $_LANG->_( 'failed' ));
	}else {
		mosErrorAlert($_LANG->_( 'Upload of' ). " $userfile_name ". $_LANG->_( 'to' )." $base_Dir ". $_LANG->_( 'successful' ), "window.close()");
	}
...
<table class="adminform">
	<tr>
	  <th class="title">
	   <?php echo $_LANG->_( 'File Upload' ); ?> : <?php echo $directory; ?>
	   </th>
	</tr>
	<tr>
	  <td align="center">
