...
function com_install()
{
	...
	$database->setQuery( "SELECT id FROM `#__languages` where `code` = 'traditional_chinese';");
	$id = $database->loadResult();
	if( empty( $id ) ) {
		$database->setQuery( "INSERT INTO `#__languages` (`name`, `active`, `iso`, `code`, `image`, `ordering`) VALUES ('Traditional Chinese', 1, 'tw', 'traditional_chinese', '', 1);");
		$database->query();	
	}
    ...
    @rename( $adminDir. "/mambots/search/jfcontent.searchbot.xml", "$mosConfig_absolute_path/mambots/search/jfcontent.searchbot_simplified_chinese.xml");
    @rename( $adminDir. "/mambots/search/jfcontent.searchbot.xml", "$mosConfig_absolute_path/mambots/search/jfcontent.searchbot_traditional_chinese.xml");
    ...
    @rename( $adminDir. "/mambots/system/jfdatabase.systembot.xml", "$mosConfig_absolute_path/mambots/system/jfdatabase.systembot_simplified_chinese.xml");
    @rename( $adminDir. "/mambots/system/jfdatabase.systembot.xml", "$mosConfig_absolute_path/mambots/system/jfdatabase.systembot_traditional_chinese.xml");
    ...
    @rename( $adminDir. "/modules/mod_jflanguageselection.xml", "$mosConfig_absolute_path/modules/mod_jflanguageselection_simplified_chinese.xml");
    @rename( $adminDir. "/modules/mod_jflanguageselection.xml", "$mosConfig_absolute_path/modules/mod_jflanguageselection_traditional_chinese.xml");
	
    // Information text
	require_once( $adminDir . "/joomfish.class.php");
	$joomFishManager = new JoomFishManager( dirname( __FILE__ ) );
	
	require_once ($mosConfig_absolute_path .'/components/com_joomfish/language/traditional_chinese.php');
