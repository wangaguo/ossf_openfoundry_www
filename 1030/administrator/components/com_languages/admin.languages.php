...
function publishLanguage( $p_lname, $option ) {
	...
	if ($fp = fopen('../configuration.php','w')){
		...
		fclose($fp);
		mosRedirect('index2.php?option=com_languages',$_LANG->_( 'Configuration succesfully updated!' ));
	} else {
		mosRedirect('index2.php?option=com_languages',$_LANG->_( 'Error! Make sure that configuration.php is writeable.' ));
	}
...
global $_LANG;
	...
	if ($fp = fopen( $file, "r" )) {
		...
		//mod by Eddy 20051027
		//mosRedirect( "index2.php?option=$option&mosmsg=".$_LANG->_( 'Operation Failed: Could not open' )." $file" );
		mosRedirect( "index2.php?option=$option",$_LANG->_( 'Operation Failed: Could not open' )." $file" );
	}
}
...
global $_LANG;
	...
	if (!$language) {
		...
		mosRedirect( "index2.php?option=$option",$_LANG->_( 'Operation failed: No language specified.' ) );
	}
	if (!$filecontent) {
		//mod by Eddy 20051027
		//mosRedirect( "index2.php?option=$option&mosmsg=".$_LANG->_( 'Operation failed: Content empty.' ) );
		mosRedirect( "index2.php?option=$option",$_LANG->_( 'Operation failed: Content empty.' ) );
	}
	...
	if (is_writable( $file ) == false) {
		...
		mosRedirect( "index2.php?option=$option",$_LANG->_( 'Operation failed: The file is not writable.' ) );
	}
	...
	if ($fp = fopen ($file, "w")) {
		...
		//mod by Eddy 20051027
		//mosRedirect( "index2.php?option=$option&mosmsg=".$_LANG->_( 'Operation failed: Failed to open file for writing.' ) );
		mosRedirect( "index2.php?option=$option",$_LANG->_( 'Operation failed: Failed to open file for writing.' ) );
	}
}
?>