...
switch ($task) {
	...
	case 'newdir':
		if (ini_get('safe_mode')=='On') {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], $_LANG->_("Directory creation not allowed while running in SAFE MODE as this can cause problems") );
		} else {
...
function create_folder($dirPath) {
	...
	if(strlen($folder_name) >0) {
		if (eregi("[^0-9a-zA-Z_]", $folder_name)) {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], $_LANG->_("Directory name must only contain alphanumeric characters and no spaces please") );
		}
