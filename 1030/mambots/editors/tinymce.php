...
function botTinymceEditorInit() {
	...
	//2007-04-24 Eddy: Change language interface by $mosConfig_lang
	//$languages = $params->def( 'languages', 'en' );

	global $mosConfig_lang;
	
	if($mosConfig_lang=='traditional_chinese')
	     $languages = $params->def( 'languages', 'tw' );
	else if($mosConfig_lang=='simplified_chinese')
	 	 $languages = $params->def( 'languages', 'cn' );
	 else
         $languages = $params->def( 'languages', 'en' );
	//Eddy:end
	...
	// Added by NCC Soft Alienchen start
	$theme_advanced_buttons4	= $params->def( 'theme_advanced_buttons4', 			0 );
	$cut						= $params->def( 'cut', 1);
	$copy           = $params->def( 'copy', 1);
	$paste          = $params->def( 'paste', 1);
	$fontselect     = $params->def( 'fontselect', 1);
	$fontsizeselect = $params->def( 'fontsizeselect', 1);
	//$forecolor      = $params->def( 'forecolor', 1);
	//$backcolor      = $params->def( 'backcolor', 1);
	// Added by NCC Soft Alienchen end
	...
	if( $theme_advanced_buttons4 > 0) {
		...
		// fontsizeselect
		if ( $fontsizeselect ) {
			$buttons4[]	= 'fontsizeselect';
		}
		/*
		// forecolor
		if ( $forecolor ) {
			$buttons4[]	= 'separator, forecolor';
		}
		// backcolor
		if ( $backcolor ) {
			$buttons4[]	= 'backcolor';
		...
		*/
		...
		$toolbar4 = 'theme_advanced_buttons4 : "' . $buttons4 . '", ';
	}
	// Added by NCC Soft Alienchen end
