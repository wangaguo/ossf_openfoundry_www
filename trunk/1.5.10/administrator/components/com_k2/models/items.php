...
class K2ModelItems extends JModel
{
	...
	function getData() {
		...
		if ($search) {
			$query .= " AND LOWER( i.title ) LIKE ".$db->Quote('%'.$search.'%');
		}
		...
		if ($filter_order == 'i.ordering') {
			$query .= " ORDER BY category, i.ordering {$filter_order_Dir}";
		} else {
			$query .= " ORDER BY {$filter_order} {$filter_order_Dir}, category, i.ordering";
		}
	...
	function getTotal() {
		...
		if ($search) {
			$query .= " AND LOWER( title ) LIKE ".$db->Quote('%'.$search.'%');
		}
	...
	function orderup() {
		...
		$row = & JTable::getInstance('K2Item', 'Table');
		$row->load($cid[0]);
		$row->move(-1, 'catid = '.(int)$row->catid.' AND published >= 0');
		$row->reorder('catid = '.(int)$row->catid.' AND published >= 0');
		$cache = & JFactory::getCache('com_k2');
		$cache->clean();
		$msg = JText::_('New ordering saved');
		$mainframe->redirect('index.php?option=com_k2&view=items', $msg);
	...
	function copy(){
		...
		foreach ($cid as $id){
			...
			JFile::copy($sourceImageXS,JPATH_ROOT.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$row->id).'_XS.jpg');
			JFile::copy($sourceImageS,JPATH_ROOT.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$row->id).'_S.jpg');
			JFile::copy($sourceImageM,JPATH_ROOT.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$row->id).'_M.jpg');
			JFile::copy($sourceImageL,JPATH_ROOT.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$row->id).'_L.jpg');
			JFile::copy($sourceImageXL,JPATH_ROOT.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$row->id).'_XL.jpg');
			JFile::copy($sourceImageGeneric,JPATH_ROOT.DS.'media'.DS.'k2'.DS.'items'.DS.'src'.DS.md5("Image".$row->id).'_Generic.jpg');
			...
			if (is_null($path)) $savepath = JPATH_ROOT.DS.'media'.DS.'k2'.DS.'attachments';
			else $savepath = $path;
			...
			foreach ($sourceAttachments as $attachment){
				...
				if (JFile::exists($savepath.DS.$attachment->filename)){
					...
					$attachmentRow->description=$attachment->description;
	...
	function featured() {
		...
		foreach ($cid as $id) {
			$row->load($id);
			if ($row->featured==1) $row->featured=0;
			else $row->featured=1;
			$db->updateObject('#__k2_items', $row, 'id');
		}
		$cache = & JFactory::getCache('com_k2');
		$cache->clean();
	...
	function import(){
		...
		foreach ($sections as $section){
			...
			$K2Category->alias = $section->alias;
			...
			foreach($categories as $category){
				...
				$K2Subcategory->alias = $category->alias;
