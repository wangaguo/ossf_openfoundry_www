...
//Modify by aeil         
        $query = "SELECT id"
                ...
                . "\n WHERE password = ''"; 
        $_CB_database->setQuery( $query ); 
        $rows = $_CB_database->loadRowList();

        foreach($rows as $v)
        {
                $query = "SELECT id FROM #__comprofiler WHERE id = ".$v[0];
                $_CB_database->setQuery( $query );
                $id_exist = $_CB_database->loadRow();
                if($id_exist == "")
                {
                        $query = "INSERT INTO #__comprofiler(id,user_id)VALUES (".$v[0].",".$v[0].");";
                        $_CB_database->setQuery( $query );//Modify by ally
                        $_CB_database->query();//Modify by ally
...
function showConfig( $option ) {
	...
	if ( ! defined( '_UE_USERNAME_OR_AUTH' ) ) {
		DEFINE('_UE_USERNAME_OR_AUTH','Username, email or enabled authentication plugins');
	}
