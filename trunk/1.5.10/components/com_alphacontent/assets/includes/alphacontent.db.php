...
/**************************************************
*  If getenv("SCRIPT_FILENAME") not works you can
*  add your absolute path. Comment this 2 lines
*  below and add $path="<your absolute path>";
***************************************************/
...
if(!$voted) {     //if the user hasn't yet voted, then vote normally...
	if (($vote_sent >= 1 && $vote_sent <= $units) && ($ip == $ip_num)) { // keep votes within range
		...
		$api_AUP = $path . '/components/com_alphauserpoints/helper.php';
		if ( file_exists($api_AUP) && $userid>0 )
		{
			require_once ($api_AUP);
			$keyreference = uniqid ( 'keyVotingAC', false );
			AlphaUserPointsHelper::newpoints( 'plgaup_voting_ac', '', $keyreference );
		}				
	} 
