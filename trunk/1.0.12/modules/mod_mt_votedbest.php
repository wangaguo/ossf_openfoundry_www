...
foreach( $listing AS $l ) {
	...
	for( $i=1; $i<=count($order); $i++ ) {
		...
		if ( $i == $order["name"] ) {
			echo '<td nowrap><a href="' . sefRelToAbs("index.php?option=com_mtree&task=viewlink&link_id=$l->link_id&Itemid=$Itemid") . '">';
		...
		if ( $i == $order["category"] )	echo '<td nowrap><a href="' . sefRelToAbs("index.php?option=com_mtree&task=listcats&cat_id=$l->cat_id&Itemid=$Itemid") . '">'. $l->category . '</a></td>';
...
if ( $show_more ) {
	...
	echo '<a href="' . sefRelToAbs("index.php?option=com_mtree&task=listtoprated&cat_id=$cat_id&Itemid=$Itemid") . '">';
