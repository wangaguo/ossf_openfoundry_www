	...
	function uninstall( $id, $option, $client=0 ) {
		...
		if (trim( $id )) {
			if (is_dir( $path )) {
				...
				HTML_installer::showInstallMessage( $_LANG->_( 'DIRECTORYDOESNOTEXIST' ), $_LANG->_( 'Uninstall error' ),
					$this->returnTo( $option, 'template', $client ) );
			}
		} else {
			HTML_installer::showInstallMessage( $_LANG->_( 'TEMPLATEIDISEMPTY' ), $_LANG->_( 'Uninstall error' ),
				$this->returnTo( $option, 'template', $client ) );
			exit();
		}
	}
