...
class mosInstaller {
	...
	function findInstallFile() {
       ...
       // Search the install dir for an xml file
		...
		if (count( $files ) > 0) {
			...
			$this->setError( 1, $_LANG->_( 'COULTNOTFINDJOOMLA' ) );
			return false;
		} else {
			$this->setError( 1, $_LANG->_( 'COULTNOTFINDXML' ) );
			return false;
		}
	}
	...
	*/
	function readInstallFile() {
      global $_LANG;
		...
		if ($root->getTagName() != 'mosinstall') {
			$this->setError( 1, $_LANG->_( 'File' ).' :"' . $this->installFilename() . '" '.$_LANG->_( 'ISNOTVALIDINSTALLATION' ) );
			return false;
		}
	...
	*/
	function install() {
	global $_LANG;
		die( $_LANG->_( 'METHODINSTALL' ).' ' . strtolower(get_class( $this )) );
	}
	/**
	...
	*/
	function uninstall() {
	global $_LANG;
		die( $_LANG->_( 'METHODUNINSTALL' ).' ' . strtolower(get_class( $this )) );
	}
	/**
	* return to method
	*/
		...
		if (is_array( $p_files ) && count( $p_files ) > 0) {
			foreach($p_files as $_file) {
				...
				if (!file_exists( $filesource )) {
					...
					$this->setError( 1, $_LANG->_( 'There is already a file called' )." $filedest - ".$_LANG->_( 'AREYOUTRYING' ) );
					return false;
				} else {
