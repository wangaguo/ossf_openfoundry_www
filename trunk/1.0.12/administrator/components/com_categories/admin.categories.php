...
function editCategory( $uid=0, $section='' ) {
	...
	if (!$sections && $type!='other') {
		...
		echo "<script> alert(".$_LANG->_('WNCATEGORY1')."; window.history.go(-1); </script>\n";
		exit();
	}
...
*/
/* Eddy: Comment 1.0.12 function and replaced by 1.0.11 function
...
function moveCategorySelect( $option, $cid, $sectionOld ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		//Add by Eddy
		header('Content-type: text/html; charset=UTF-8');
		echo "<script> alert('".$_LANG->_( 'SELECT_ITEM_TO_MOVE' )."'); window.history.go(-1);</script>\n";
		exit;
	}
...
function moveCategorySave( $cid, $sectionOld ) {
	...
	$msg = ( (count($cid) - 1) ? $_LANG->_( 'Categories moved to' ) :$_LANG->_( 'Categories moved to' )).' '. $sectionNew->name;
	//mod by Eddy 20051027
...
function copyCategorySave( $cid, $sectionOld ) {
    ...
    $msg = ( (count($cid) - 1) ? $_LANG->_( 'Categories copied to' ) : $_LANG->_( 'Categories copied to' ) ) . $sectionNew->name;
