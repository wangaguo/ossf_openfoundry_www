...
// check to see if session id corresponds with correct format
		...
		// check against db record of session
				...
				if ( $count == 0 ) {
                    ...
                    //mod by Eddy using javascript fuction encodeURIComponent()
					//echo "<script>document.location.href='index.php?mosmsg=Admin Session Expired'</script>\n";
					echo "<script>document.location.href='index.php?mosmsg='+encodeURIComponent('".$_LANG->_('Admin Session Expired')."')</script>\n";
					exit();
} else {
			...
			echo "<script>document.location.href='index.php?mosmsg='+encodeURIComponent('".$_LANG->_('You need to login')."')</script>\n";
			exit();
		} else {
			...
			echo "<script>document.location.href='index.php?mosmsg='+encodeURIComponent('".$_LANG->_('Invalid Session')."')</script>\n";
			exit();
		}
	...
	*/
	function getPath( $varname, $option='' ) {
		global $mosConfig_absolute_path;
		//Add by Eddy for multi_xml load
			...
			switch ($varname) {
				case 'com_xml':
					...
					if($mosConfig_lang!='english'){
						$lang_xml_path = "$mosConfig_absolute_path/administrator/components/$option/$name".'_'.$mosConfig_lang.'.xml';

					if(file_exists($lang_xml_path))
						$path=$lang_xml_path;
					}
					//End
				...
				case 'mod0_xml':
					...
					if ($option == '') {
						...
						//Add by Eddy for multi_xml load
					    if($mosConfig_lang!='english'){
					    	$lang_xml_path = $mosConfig_absolute_path."/modules/custom".'_'.$mosConfig_lang.'.xml';
						...
						//Add by Eddy for multi_xml load
					    if($mosConfig_lang!='english'){
					    	$lang_xml_path = $mosConfig_absolute_path."/modules/$option".'_'.$mosConfig_lang.'.xml';
				...
				case 'mod1_xml':
					...
					if ($option == '') {
						...
						//Add by Eddy for multi_xml load
					    if($mosConfig_lang!='english'){
					    	$lang_xml_path = $mosConfig_absolute_path."/administrator/modules/custom".'_'.$mosConfig_lang.'.xml';
						...
						//Add by Eddy for multi_xml load
					    if($mosConfig_lang!='english'){
					    	$lang_xml_path = $mosConfig_absolute_path."/administrator/modules/$option".'_'.$mosConfig_lang.'.xml';
				...
				case 'menu_xml':
					...
					global $mosConfig_lang;
					if($mosConfig_lang!='english'){
						$lang_xml_path = $mosConfig_absolute_path . "/administrator/components/com_menus/$option/$option".'_'.$mosConfig_lang.'.xml';
...
function mosFormatDate( $date, $format="", $offset=NULL ){
    ...
    //mod by Eddy 20060717
    global $mosConfig_locale;
    
    if(stristr($mosConfig_locale,"UTF8")==false)
	   $date=default_to_utf8($date);
...
function mosCreateMail( $from='', $fromname='', $subject, $body ) {
   ...
   //mod by Eddy 20060717
	  $mail->FromName = utf8_to_default($mail_fromname);
	//end mod
    ...
    //mod by Eddy 20051024
	  $mail->Subject = utf8_to_default($subject);
	  $mail->Body 	=  utf8_to_default($body);
	//end mod 
	...
	*/
	function MenuLinks( &$lookup, $all=NULL, $none=NULL, $unassigned=1 ) {
		...
		if ( $none ) {
			...
			//Mod by Eddy
			$mitems[] = mosHTML::makeOption( -999, _CMN_NONE );
		...
		if ( $unassigned ) {
			...
			//Mod by Eddy
			$mitems[] = mosHTML::makeOption( 99999999, _CMN_UNASSIGNED );
			// adds space, in select box which is not saved
			$mitems[] = mosHTML::makeOption( -999, '----' );
		}
...
class mosCommonHTML {
	...
	*/
	function loadCalendar() {
		...
		<?php 
			//2007-04-17 Eddy: language change 
			global $mosConfig_lang;
			if($mosConfig_lang=='traditional_chinese'){
		?>
		
		<script type="text/javascript" src="<?php echo $mosConfig_live_site;?>/includes/js/calendar/lang/calendar-tw.js"></script>
		
		<?php }else if($mosConfig_lang=='simplified_chinese'){ ?>		
		
		<script type="text/javascript" src="<?php echo $mosConfig_live_site;?>/includes/js/calendar/lang/calendar-cn.js"></script>
...
function mosSmartSubstr($text, $length=200, $searchword) {
	global $mosConfig_absolute_path;
  //add by Eddy 2006.07.17
  require_once( $mosConfig_absolute_path . '/includes/utf8.php' );
	
  $wordpos = utf8_strpos(strtolower($text), strtolower($searchword));
  $halfside = intval($wordpos - $length/2 - utf8_strlen($searchword));
  if ($wordpos && $halfside > 0) {
	return '...' . utf8_substr($text, $halfside, $length) . '...';
  } else {
...
//add By Eddy for UTF-8 to BIG5,GB2312 encoding convertion
function utf8_to_default($str){
	global $mosConfig_lang,$mosConfig_absolute_path;
	
    if($mosConfig_lang== 'traditional_chinese'){
    	if(function_exists('iconv')){
    		$result=iconv( "UTF-8", "BIG5", $str );
        }else if(function_exists('mb_convert_encoding')){
        	$result=mb_convert_encoding($str, "BIG5", "UTF-8");
	    }else{
	        require_once($mosConfig_absolute_path.'/includes/big5_func/big5_func.inc.php');
	         $result=big5_utf8_decode($str);
	    }
	}elseif($mosConfig_lang== 'simplified_chinese'){
	  if(function_exists('iconv')){
    		$result=iconv( "UTF-8", "GB2312", $str );
        }else if(function_exists('mb_convert_encoding')){
        	$result=mb_convert_encoding($str, "GB2312", "UTF-8");
	    }else{
	        require_once($mosConfig_absolute_path.'/includes/big5_func/big5_func.inc.php');
	         $result=big5_utf8_decode($str);
	         $result=big52gb($result);
	    }
	}else{
	  $result=$str;
	}	
...
function default_to_utf8($str){
	global $mosConfig_lang,$mosConfig_absolute_path;
	
    if($mosConfig_lang== 'traditional_chinese'){
    	if(function_exists('iconv')){
    		$result=iconv( "BIG5", "UTF-8", $str );
        }else if(function_exists('mb_convert_encoding')){
        	$result=mb_convert_encoding($str, "UTF-8", "BIG5");
	    }else{
	        require_once($mosConfig_absolute_path.'/includes/big5_func/big5_func.inc.php');
	         $result=big5_utf8_encode($str);
	    }
	}elseif($mosConfig_lang== 'simplified_chinese'){
	  if(function_exists('iconv')){
