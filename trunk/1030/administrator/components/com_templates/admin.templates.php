...
function editTemplateSource( $p_tname, $option, $client ) {
	...
	if ( $fp = fopen( $file, 'r' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, $_LANG->_( 'Operation Failed: Could not open' ). $file );
	}
}
...
function saveTemplateSource( $option, $client ) {
	...
	if ( !$template ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, $_LANG->_( 'Operation failed: No template specified.' ) );
	}
	if ( !$filecontent ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, $_LANG->_( 'Operation failed: Content empty.' ) );
	}
	...
	if ( $fp = fopen ($file, 'w' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, $_LANG->_( 'Operation failed: Failed to open file for writing.' ) );
	}
...
function saveTemplateCSS( $option, $client ) {
	...
	clearstatcache();
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, $_LANG->_( 'Operation failed: The file is not writable.') );
	}
