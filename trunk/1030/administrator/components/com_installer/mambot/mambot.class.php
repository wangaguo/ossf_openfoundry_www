...
class mosInstallerMambot extends mosInstaller {
	...
	*/
	function uninstall( $id, $option, $client=0 ) {
		...
		if ($row == null) {
			HTML_installer::showInstallMessage( $_LANG->_( 'Invalid object id' ), $_LANG->_( 'Uninstall error' ),
			$this->returnTo( $option, 'mambot', $client ) );
			exit();
		}

		if (trim( $row->folder ) == '') {
			HTML_installer::showInstallMessage( $_LANG->_( 'FOLDERFIELDEMPTY' ), $_LANG->_( 'Uninstall error' ),
			$this->returnTo( $option, 'mambot', $client ) );
			exit();
		}
		...
		if ($row->iscore) {
			HTML_installer::showInstallMessage( $row->name .' '.$_LANG->_( 'ISACOREELEMENT' ),
