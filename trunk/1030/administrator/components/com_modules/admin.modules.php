...
function removeModule( &$cid, $option, $client ) {
	...
	foreach ($rows as $row) {
		...
		if ( $row->module == 'mod_mainmenu' ) {
			if ( strstr( $row->params, 'mainmenu' ) ) {
				//Add by Eddy
				@header('Content-type: text/html; charset=UTF-8');
				echo "<script> alert('".$_LANG->_( 'WNDELETEMAINMENU' )."'); window.history.go(-1); </script>\n";
				exit;
			}
		}
	}
	...
	if (count( $err )) {
		...
		//Add by Eddy
		@header('Content-type: text/html; charset=UTF-8');
		echo "<script>alert('".$_LANG->_( 'Module(s)' ).": \'$cids\' ".$_LANG->_( 'WARNCANNOTBEDELETEDMAMBOMODULES' )."');</script>\n";
	}
