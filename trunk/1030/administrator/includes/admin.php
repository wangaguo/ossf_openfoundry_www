...
function josSecurityCheck($width='95%') {
	...
	if ( ini_get('magic_quotes_gpc') != '1' ) {
		$wrongSettingsTexts[] = $_LANG->_("PHP magic_quotes_gpc setting");
	}
	if ( ini_get('register_globals') == '1' ) {
		$wrongSettingsTexts[] = $_LANG->_("PHP register_globals setting");
	}
	if ( RG_EMULATION != 0 ) {
		$wrongSettingsTexts[] = $_LANG->_("Joomla! RG_EMULATION setting").'<br /><span style="font-weight: normal; font-style: italic; color: #666;">'.$_LANG->_("Joomla! RG_EMULATION setting2").'</span>';
	}	
	...
	if ( count($wrongSettingsTexts) ) {
		...
		<div style="clear: both; margin: 3px; margin-top: 10px; padding: 5px 15px; display: block; float: left; border: 1px solid #cc0000; background: #ffffcc; text-align: left; width: <?php echo $width;?>;">
			<p style="color: #CC0000;">
				<?php echo $_LANG->_("Following PHP Server Settings");?>
			</p>			
			...
			<p style="color: #666;">
				<?php echo $_LANG->_('Please check');?> <a href="http://www.joomla.org/security10" target="_blank" style="color: blue; text-decoration: underline"><?php echo $_LANG->_('the Official Joomla! Server Security post');?></a> <?php echo $_LANG->_('for more information.');?>
			</p>
		</div>
		<?php
	}
