...
class HTML_content {
	...
	*/
	function Title( &$row, &$params, &$access ) {
		if ( $params->get( 'item_title' ) ) {
			if ( $params->get( 'link_titles' ) && $row->link_on != '' ) {
				...
				?>
				<td class="contentheading<?php echo $params->get( 'pageclass_sfx' ); ?>" width="100%" style="font-size: 16px;font-weight:bold;vertical-align:bottom;color: #0055C5;" >
	...
	/**
	* Writes CatalogTitle by ally 2007/06/20
	*/
	function CalTitle( &$row, &$params, &$access ) {
		...
		if ( $params->get( 'item_title' ) ) {
				...
				<img src="http://www.openfoundry.org/images/newsletter/icon-cat.gif" align="left" hspace="6" alt="Image" /><a href="<?=$url?>#<?php echo $row->id ; ?>" class="contentpagetitle<?php echo $params->get( 'pageclass_sfx' ); ?>">
	...
	*/
	function Author( &$row, &$params ) {
		if ( ( $params->get( 'author' ) ) && ( $row->author != '' ) ) {
			?>
			<tr>
				<td width="70%" align="left" valign="top" colspan="2" class="small">
					<!--<span class="small">-->
	...
	/**
	* For Newsletter Author add HTML style
	* by ally 2007/06/28
	**/
	function IMAuthor (&$row, &$params ) {
		if( ( $params->get( 'author' ) ) && ( $row->author != '') ) {
		?>
			<tr>
				<td width="70%" height="30" align="left" valign="top" colspan="2" style="font-size:12px;color:#999;font-weight:bold">
						<?php echo ( $row->created_by_alias ? $row->created_by_alias : $row->author ); ?>
	...
	*/
	function CreateDate( &$row, &$params ) {
		...
		if ( $params->get( 'createdate' ) ) {
			...
			<!--<span class="small">--><?php echo $create_date; ?><!--</span>-->
				</td>
			</tr>
			<?php
		}
	...
	/**
	* for Newsletter add HTML style
	* by ally 2007/06/28
	**/
	function IMCreateDate( &$row, &$params ) {
		$create_date = null;
