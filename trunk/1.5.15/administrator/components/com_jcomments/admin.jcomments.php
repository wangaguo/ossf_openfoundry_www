...
// add by max 201001 ## begin ## 
        // lastest articles
        // catid of allow comments

		$whereCatid = '';
        $lists['latestArticles'] = array();
		if ($config->get('enable_categories') != '') {
			$catids = explode( ',', $config->get('enable_categories') );
			//mosArrayToInts( $catids );
			$whereCatid = "\n AND ( a.catid=" . implode( " OR a.catid=", $catids ) . " )";
         
        $nullDate 	= $db->getNullDate();
        
		$query = "SELECT a.id, a.title, a.introtext, a.created, u.name as author,a.sectionid, a.catid, CONCAT(s.title,'/',cc.title) as catetory, cc.access AS cat_access, s.access AS sec_access, cc.published AS cat_state, s.published AS sec_state"
		. "\n FROM #__content AS a"
		...
		. "\n WHERE a.state = 1"
		. "\n AND ( a.publish_up = " . $db->Quote( $nullDate ) . " OR a.publish_up <= " . $db->Quote( _CURRENT_SERVER_TIME ) . " )"
		. "\n AND ( a.publish_down = " . $db->Quote( $nullDate ) . " OR a.publish_down >= " . $db->Quote( _CURRENT_SERVER_TIME ) . " )"
		. ( $access ? "\n AND a.access <= " . (int) $my->gid : '' )
		. $whereCatid
		. ( $show_front == '0' ? "\n AND f.content_id IS NULL" : '' )
		. "\n ORDER BY a.created DESC"
        ;
		$db->setQuery( $query, 0, 50 ); // load 30 at most
		...
		if (count($temp)) {
			foreach ($temp as $row ) {
				if (($row->cat_state == 1 || $row->cat_state == '') &&  ($row->sec_state == 1 || $row->sec_state == '') &&  ($row->cat_access <= $my->gid || $row->cat_access == '' || !$access) &&  ($row->sec_access <= $my->gid || $row->sec_access == '' || !$access)) {
                    if(strstr($row->introtext, '{jcomments off}')===false) {
		...
		unset($temp);
        $lists['latestArticles'] = $rows;
        }  
        // authors
        $auids=explode(',',$config->get('disabled_notification_authors'));
        $query = "SELECT id, name, email FROM #__users WHERE (usertype = 'Manager' OR usertype = 'Administrator' OR usertype = 'Super Administrator' OR usertype='Author') and block=0";
        $db->setQuery( $query); // load 30 at most
        $lists['authors'] = $db->loadObjectList();
        foreach($lists['authors'] as &$au) {
            if(in_array($au->id, $auids)) {
                $au->notification = false;
            } else {
                $au->notification = true;
            }
        }

        // notification categories

        $query = "SELECT c.id AS id, CONCAT_WS( ' / ', s.title, c.title) AS `catName`"
            . "\n FROM #__sections AS s"
            . "\n INNER JOIN #__categories AS c ON c.section = s.id"
            . "\n ORDER BY s.name,c.name"
            ;
        $db->setQuery( $query );
        $rows = $db->loadObjectList();
        $cats = explode('|', $config->get('notification_emails_of_categories'));
        $arrCat = array();
        if(count($cats)>0) {
            foreach($cats as $c) {
                $pos = strpos($c, ':');
                $cid = substr($c, 0, $pos);
                $emails = substr($c, $pos+1);
                $arrCat[$cid] = $emails;
            }
        }
        foreach($rows as &$r) {
            $r->emails = isset($arrCat[$r->id]) ? $arrCat[$r->id] : '';
        }
        $lists['noti_categories'] = $rows;

// add by max 201001 ## end ## 
...
// add by max 201001 ## begin ## 
        // set commentable articles
        foreach($_POST['commentid'] as $cid => $commentable) {
            $query = "SELECT introtext from #__content WHERE id=$cid";
			$db->setQuery($query);
			$rows = $db->loadObjectList();
            $t = $rows[0]->introtext;
            if($commentable) {
                // remove all {jcomments off} from introtext
                $tlen = strlen('{jcomments off}');
                while(1) {
                    $pos = strpos($t, '{jcomments off}');
                    if($pos===false) { break; } 
                    $t = substr($t, 0,$pos). substr($t, $pos+$tlen);
                }
            } else {
                // add {jcomments off} to the end of introtext
                $t = $t.'{jcomments off}';
            }
            $query = "UPDATE #__content SET introtext='$t' WHERE id=$cid";
			$db->setQuery($query);
			$db->query();
        }

        // save author's id who DON'T want to receive notification
        $arrAid = array();
        foreach($_POST['authorid'] as $aid => $notification) {
            if(!$notification) {
                $arrAid[] = $aid;
            }
        }
        $allId = implode(',',$arrAid);
        $query = "DELETE FROM #__jcomments_settings WHERE name='disabled_notification_authors' " .  ($lang != '' ? " AND `lang` = '$lang'" : '' );
        $db->setQuery($query);
        $db->query();
        $query = "INSERT INTO #__jcomments_settings SET name='disabled_notification_authors', value='" . $allId . "' " .  ($lang != '' ? " , `lang` = '$lang'" : '' );
		$db->setQuery($query);
     	$db->query();

        // save notfication emails for categories
        $arrCat = array();
        foreach($_POST['notiCat'] as $cid => $emails) {
            if($emails!='') {
                $arrCat[] = $cid.':'.$emails;
            }
        }
        $query = "DELETE FROM #__jcomments_settings WHERE name='notification_emails_of_categories' " .  ($lang != '' ? " AND `lang` = '$lang'" : '' );
        $db->setQuery($query);
        $db->query();
        $query = "INSERT INTO #__jcomments_settings SET name='notification_emails_of_categories', value='" . implode('|', $arrCat) . "' " .  ($lang != '' ? " , `lang` = '$lang'" : '' );
		$db->setQuery($query);
        $db->query();
