...
<!-- add by max 201001 ## begin ## -->
    ...
    <tr bgcolor="#CCCCCC">
        <td colspan="3"><?php echo JText::_('AP_NOTIFICATION_CATEGORY_RECEIVER') ?></td>
    </tr>
    <tr bgcolor="#CCCCCC">
        <td>Enabled categories</td>
        <td colspan="2">E-mails for notification (separated by comma ',')</td>
    </tr>
<?php 
foreach ($lists['noti_categories'] as $nc) {?>
    <tr>
        <td><?php echo $nc->catName; ?></td>
        <td colspan="2"><input type="edit" size="100" name="notiCat[<?php echo $nc->id;?>]" value="<?php echo $nc->emails;?>" ></td>
    </tr>
<?php } ?>
</table>
...
<?php
		$tabs->endTab();
		$tabs->startTab(JText::_('A_LATEST'), "Latest articles");
?>
<fieldset class="adminform"><legend><?php echo JText::_('A_LATEST_ALLOW'); ?></legend>
<table class="adminlist" cellspacing="1">
	<thead>
		<tr>
			<th width="2%" class="title">#</th>
            <th class="title">Title</th>
			<th width="10%" class="title">Commentable?</th>
            <th width="2%"> ID </th>
			<th align="left" width="16%"> Category </th>
			<th align="left" width="8%"> Author </th>
			<th align="right" width="7%"> Date </th>
		</tr>
    </thead>
    <tbody>
<?php   
        $k=1;        
foreach($lists['latestArticles'] as $la) { 
?>
    	<tr align="left">
            <td><?php echo $k;?></td>
            <td><?php echo $la->title;?></td>
            <td><?php echo JCommentsHTML::yesnoRadioList( 'commentid['.$la->id.']', NULL, $la->commentable, JText::_('A_YES'), JText::_('A_NO') ); ?></td>
            <td align="right"><?php echo $la->id;?></td>
            <td><?php echo $la->catetory;?></td>
            <td><?php echo $la->author;?></td>
            <td><?php echo JCommentsText::formatDate(strtotime($la->created), '%Y-%m-%d'); ?></td>
    </tr>
<?php 
    $k++; 
}
...
<?php
		$tabs->endTab();
		$tabs->startTab(JText::_('A_AUTHOR'), "Authors");
?>
<fieldset class="adminform"><legend><?php echo JText::_('A_AUTHOR_NOTI'); ?></legend>
<table class="adminlist" cellspacing="1">
	<thead>
		<tr>
			...
			<th width="20%" class="title">Enable notification?</th>
		</tr>
    </thead>
    <tbody>
<?php   
...
?>
    	<tr align="left">
            <td><?php echo $k;?></td>
            <td align="right"><?php echo $au->id;?></td>
            <td><?php echo $au->name;?></td>
            <td><?php echo $au->email;?></td>
            <td><?php echo JCommentsHTML::yesnoRadioList( 'authorid['.$au->id.']', NULL, $au->notification, JText::_('A_YES'), JText::_('A_NO') ); ?></td>
    </tr>
<?php 
    $k++; 
}
