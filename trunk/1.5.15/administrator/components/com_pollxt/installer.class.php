...
<?php
class xtInstaller {
	function installPlugins($installer) {
...
if (is_a($plugins, 'JSimpleXMLElement') && count($plugins->children())) {
	...
	foreach ($plugins->children() as $plugin)
	{
		...
		if (!empty($pname) && !empty($pgroup)) {
			$installer->parent->setPath('extension_root', JPATH_ROOT.DS.'plugins'.DS.$pgroup);
		} else {
			$installer->parent->abort(JText::_('Plugin').' '.JText::_('Install').': '.JText::_('No plugin file specified'));
			return false;
		}
		...
		if (!file_exists($installer->parent->getPath('extension_root'))) {
			if (!$created = JFolder::create($installer->parent->getPath('extension_root'))) {
				$installer->parent->abort(JText::_('Plugin').' '.JText::_('Install').': '.JText::_('Failed to create directory').': "'.$installer->parent->getPath('extension_root').'"');
				return false;
			}
		}
	...
	function __construct($table) {
		$db = JFactory::getDBO();
		$this->pre = $db->getPrefix();
		$this->upgrade = $this->tableExists($table);
	}	
	...
	function createTable($cTable) {
	 	$success = true;
	 	$output = "";
	 	$db = JFactory::getDBO();
		foreach ($cTable as $t) {
			$query = "SHOW CREATE TABLE ".$this->pre.$t->tname;
			$db->setQuery($query);
			$result = $db->loadObject();

			$create = "Create Table";
			if (isset($result->$create)) $dbstruct = $result->$create;
			else $dbstruct = "";
			$inststruct = str_replace ("#__", $this->pre, $t->tstruct);
			
			$updater = new dbStructUpdater();
			$res = $updater->getUpdates($dbstruct, $inststruct);
			foreach ($res as $r) {
                $db->setQuery(str_replace($this->pre, "#__", $r));
                $db->query();
                if ($db->getErrorNum()) {
                    $output .= "<tr><td style=\"color:#bb0000; font-weight:bold; font-size:12px\">";
                    $output .= $db->getErrorMsg()."</td></tr>";
                    $success = false;
                    echo $db->getErrorMsg();
                }
                else {
                    $output .= "<tr><td style=\"color:#009900; font-weight:bold; font-size:12px\">";
                    $output .= "Upgrade Table ".$t->tname." successful</td></tr>"; 
				}
			}
		}
		$this->output = $output;
...
class dbStructUpdater
{
	...
	*/
	...
	function init()
	{
		...
		$this->config['updateTypes'] = 'create, drop, add, remove';
	...
	*/
	...
	{
		$result = '';
		/* create table should be single line in installer case*/
	...
	*/
	...
	/**
	* Searches for the position of the next delimiter which is not inside string literal like 'installer ; ' or
	* like "installer ; ".
	*
