 ...
 * @since 0.9
 */
class EventListViewMailLetter extends JView {
	...
	function display($tpl = null)
	{
		...
		if ( $cid ) {
			JToolBarHelper::title( JText::_( 'EDIT MAIL' ), 'editmail' );
		} else {
			JToolBarHelper::title( JText::_( 'ADD MAIL' ), 'editmail' );
		}
		...
		if ($row->id) {
			if ($model->isCheckedOut( $user->get('id') )) {
				...
				$mainframe->redirect( 'index.php?option=com_eventlist&view=mail_list' );
			}
		}
