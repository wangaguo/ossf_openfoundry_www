...
define('_COM_SEF_SH_CB_USE_USER_PSEUDO', 'Вставить Алиас пользователя');
...
define('_COM_SEF_SH_FB_TITLE', 'Параметры Fireboard ');
...
define('_COM_SEF_SH_IJOOMLA_MAG_TITLE', 'Параметры Магазина iJoomla');
...
define('_COM_SEF_SH_REDIRECT_JOOMLA_SEF_TO_SEF', 'Перенаправление (код 301) с Joomla SEF в sh404SEF');
define('_COM_SEF_SH_REDIRECT_NON_SEF_TO_SEF', 'Перенаправление (код 301) из НЕ-Sef в SEF URL');
...
define('_COM_SEF_SH_USE_URL_CACHE', 'Включить кэш ссылок (URL)');
...
define('_COM_SEF_SH_VM_INSERT_FLYPAGE', 'Вставить Имя flypage');
...
define('_COM_SEF_TITLE_ADV','Дополнительные Параметры компонента');
...
define('_COM_SEF_TT_ADV4','Дополнительные Параметры для ');
...
define('_COM_SEF_TT_SH_REDIRECT_JOOMLA_SEF_TO_SEF', 'Если <strong>Да</strong>, то стандартные ссылки JOOMLA SEF будут перенаправлены по коду 301 в sh404SEF эквивалент, если они есть в БД. Если их нет в БД, то они будут созданы налету.');
define('_COM_SEF_TT_SH_REDIRECT_NON_SEF_TO_SEF', 'Если <strong>Да</strong>, не-Sef URL уже присутствующие в БД будут перенаправлены в их SEF часть используя код 301 - Постоянно перемещенное перенаправление. Если SE-URL не существует, оно будет создано, если кроме этого там есть POST информация в запросе страницы.');
...
define('_COM_SEF_TT_USE_ALIAS','Выберите Да, чтобы использовать Алиасы зоголовков вместо Текста заголовка в ссылке (URL)');
...
define('_COM_SEF_USE_ALIAS','Использовать Алиасы');
define('_COM_SEF_USE_DEFAULT','Заголовок по умолчанию');
...
define('_COM_SEF_SH_RW_MODE_NORMAL', '.htaccess ( mod_rewrite )');
...
define('_COM_SEF_SH_CONF_META_DOC', 'sh404SEF имеет несколько плагинов для <strong>автоматического</strong> создания META тегов для некоторых компонентов.<br/>Не создавайте их вручную, пока автоматически создаваемые не перестануть Вас удовлетворять!<br>');
...
define('_COM_SEF_SH_REDIRECT_WWW', 'Перенаправление www/не-www (код 301)');
define('_COM_SEF_TT_SH_REDIRECT_WWW', 'Если Да, то sh404SEF будет выполнять перенаправление (код 301), если название сайта запрошено без www. Если URL сайта начинается с www, или если сайт запросшен с www, в то время, как его название без www - это предотвратит дублирование содержания и некоторые проблемы, зависящие от конфигурации вашего сервера Apache, такие как проблемы с WYSYWIG редакторами в Joomla');
...
define('_COM_SEF_SH_UPGRADE_TITLE', 'Сохранение конфигурации');
...
// v x
define('_COM_SEF_SH_UPGRADE_KEEP_CONFIG', 'Сохраненить конфигурацию?');
...
define('_COM_SEF_STANDARD_ADMIN', 'Перейти в режим Минимального отображения (только основные параметры)');
define('_COM_SEF_ADVANCED_ADMIN', 'Перейти в режим Расширенного отображения (все доступные параметры)');
define('_COM_SEF_SH_MULTIPLE_H1_TO_H2', 'Множестенное изменение h1 в h2');
define('_COM_SEF_TT_SH_MULTIPLE_H1_TO_H2', 'Если Да и есть несколько тегов h1 на странице, то они будут преобразованы в теги h2.<br />Если же на странице только один тег h1, то он будет оставлен, как есть.');
define('_COM_SEF_SH_INSERT_NOFOLLOW_PDF_PRINT', 'Вставлять тег nofollow в ссылки Печать и PDF');
define('_COM_SEF_TT_SH_INSERT_NOFOLLOW_PDF_PRINT', 'Если Да, то атрубуты rel=nofollow будут добавлены ко всем ссылкам Печать и PDF, созданным Joomla. Это позволит снизить дублирование содержимого, видимого поисковиками.');
define('_COM_SEF_SH_INSERT_READMORE_PAGE_TITLE', 'Вставлять Заголовок в ссылки Подробнее...');
define('_COM_SEF_TT_SH_INSERT_READMORE_PAGE_TITLE', 'Если Да и ссылка Подробнее... отображена на странице, то соответствующий содержимому заголовок будет добавлен к ссылке для увелиения ее веса в поисковиках.');
define('_COM_SEF_VM_USE_ITEMS_PER_PAGE', 'Использование Элементов на постраничном ниспадающем списке');
define('_COM_SEF_TT_VM_USE_ITEMS_PER_PAGE', 'Если Да, то ссылки будут настроены так, чтобы, используя ниспадающие списки, пользователь мог выбрать номер элементов постранично. Если не используете такого рода ниспадающие списки, ИЛИ ваши ссылки уже проиндексированы поисковой системой, вы можете установить данный параметр в НЕТ для сохранения ваших существующих ссылок.');
define('_COM_SEF_SH_CHECK_POST_DATA', 'Проверять также формы данных (POST)');
...
define('_COM_SEF_SH_SEC_STATS_TITLE', 'Статус Безопасности');
define('_COM_SEF_SH_SEC_STATS_UPDATE', 'Обновить');
define('_COM_SEF_SH_TOTAL_ATTACKS', 'Насчитано атак');
define('_COM_SEF_SH_TOTAL_CONFIG_VARS', 'mosConfig var в ссылке');
define('_COM_SEF_SH_TOTAL_BASE64', 'Base64 инъекций');
define('_COM_SEF_SH_TOTAL_SCRIPTS', 'Script инъекций');
define('_COM_SEF_SH_TOTAL_STANDARD_VARS', 'Направомерные стандартные переменные (vars)');
define('_COM_SEF_SH_TOTAL_IMG_TXT_CMD', 'Дистанционное включение файла');
define('_COM_SEF_SH_TOTAL_IP_DENIED', 'Запрещенных IP адресов');
define('_COM_SEF_SH_TOTAL_USER_AGENT_DENIED', 'Запрещенных User agent`ов');
define('_COM_SEF_SH_TOTAL_FLOODING', 'Слишком много запросов (флуд)');
define('_COM_SEF_SH_TOTAL_PHP', 'Отвергнутых Проектом Honey Pot');
define('_COM_SEF_SH_TOTAL_PER_HOUR', ' /h');
define('_COM_SEF_SH_SEC_DEACTIVATED', 'Функции безопасности НЕ активированы');
define('_COM_SEF_SH_TOTAL_PHP_USER_CLICKED', 'PHP, но по нажатию пользователей');
define('_COM_SEF_SH_COM_SMF_TITLE', 'Параметры моста SMF');
define('_COM_SEF_SH_INSERT_SMF_NAME', 'Вставить Название Форума');
define('_COM_SEF_TT_SH_INSERT_SMF_NAME', 'Если <strong>Да</strong>, то заголовок элемента меню предшествующий Главной странице форума будет добавлен ко всем форумным SEF ссылкам.');
define('_COM_SEF_SH_SMF_ITEMS_PER_PAGE', 'Элементов на странице');
define('_COM_SEF_TT_SH_SMF_ITEMS_PER_PAGE', 'Кол-во элементов отображаемых на одной странице форума.');
define('_COM_SEF_SH_INSERT_SMF_BOARD_ID', 'Вставить ID форума');
define('_COM_SEF_TT_SH_INSERT_SMF_BOARD_ID', _COM_SEF_TT_SH_FB_INSERT_CATEGORY_NAME);
define('_COM_SEF_SH_INSERT_SMF_TOPIC_ID', 'Вставить ID Темы');
define('_COM_SEF_TT_SH_INSERT_SMF_TOPIC_ID', _COM_SEF_TT_SH_FB_INSERT_MESSAGE_ID);
define('_COM_SEF_SH_INSERT_SMF_USER_NAME', 'Вставить имя пользователя');
define('_COM_SEF_TT_SH_INSERT_SMF_USER_NAME', 'Если <strong>Да</strong>, то имя пользователя будет добавлено в каждую соответствующую ссылку вместо его ID. Это займет место в БД, так как создается уникальный URL для каждого пользователя и каждой функции (просмотр профиля и т.д.)');
define('_COM_SEF_SH_INSERT_SMF_USER_ID', 'Вставить ID пользователя');
define('_COM_SEF_TT_SH_INSERT_SMF_USER_ID', 'Если <strong>Да</strong>, то имени пользователя все время будет предшествовать его ID, чтобы быть уверенным, что оно уникально.');
define('_COM_SEF_SH_PREPEND_TO_PAGE_TITLE', 'Вставлять перед заголовком страницы');
define('_COM_SEF_TT_SH_PREPEND_TO_PAGE_TITLE', 'Текст, введенный здесь, будет добавлен ко всем тегам заголовка страницы.');
define('_COM_SEF_SH_APPEND_TO_PAGE_TITLE', 'Добавить к заголовку страницы');
define('_COM_SEF_TT_SH_APPEND_TO_PAGE_TITLE', 'Текст, введенный здесь, будет добавлен ко всем тегам заголовка страницы.');
define('_COM_SEF_SH_DEBUG_TO_LOG_FILE', 'Записывать Лог в файл');
define('_COM_SEF_TT_SH_DEBUG_TO_LOG_FILE', 'Если Да, то sh404SEF будет записывать Лог в текстовый файл множество внутренней информации. Данная информация необходима для анализа решения проблем работы sh404SEF. <br/>ВНИМАНИЕ: данный файл может быстро стать большим. Также побочным эффектом данной функции является снижение скорости работы сайта. Используйте данную функцию только когда это действительно необходимо. В этой связи, данная функция автоматически отключается через час после включения. Выкл./вкл. для повторной ее активации. Лог файл находится здесь: /administrator/components/com_sh404sef/logs/ ');

define('_COM_SEF_ALIAS_LIST', 'Список Алиасов');
define('_COM_SEF_TT_ALIAS_LIST', 'Добавьте сюда список алиасов для Данной ссылки. По одному алиасу на строку, т.е.:<br/>old-url.html<br/>или<br/>my-other-old-url.php?var=12&test=15<br> и sh404SEF сделает перенаправление (код 301) на верную SEF ссылку, если какой-либо из введенных алиасов будет запрошен.');
define('_COM_SEF_HOME_ALIAS', 'Алиас Главной страницы');
define('_COM_SEF_TT_HOME_PAGE_ALIAS_LIST', 'Добавьте сюда список алиасов для Главной страницы. По одному алиасу на строку, т.е.:<br/>old-url.html<br/>или<br/>my-other-old-url.php?var=12&test=15<br>и sh404SEF сделает перенаправление (код 301) на Главную страницу, если какой-либо из алиасов будет запрошен.');
define('_COM_SEF_SH_INSERT_OUTBOUND_LINKS_IMAGE', 'Вставить символ Внешних ссылок');
define('_COM_SEF_TT_SH_INSERT_OUTBOUND_LINKS_IMAGE', 'Если Да, то визуализирующий символ будет добалвен ко всем ссылкам, ссылающимся на другой сайт, для обеспечения легкой идентификации этих ссылок.');
define('_COM_SEF_SH_OUTBOUND_LINKS_IMAGE_BLACK', 'Темный');
define('_COM_SEF_SH_OUTBOUND_LINKS_IMAGE_WHITE', 'Светлый');
define('_COM_SEF_SH_OUTBOUND_LINKS_IMAGE', 'Оттенок символа Внешних ссылок');
define('_COM_SEF_TT_SH_OUTBOUND_LINKS_IMAGE', 'Обе картинки содержат полупрозрачный фон. Выберите темный, если фон сайта светлый. Если же фон светлый, выберите темный символ. Картинки находятся здесь: /administrator/components/com_sef/images/external-white.png и external-black.png. Из размер 15x16 пикселей.');

// V 1.3.3
define('_COM_SEF_DEFAULT_PARAMS_TITLE', 'Специализированное');
define('_COM_SEF_DEFAULT_PARAMS_WARNING', 'ВНИМАНИЕ: изменяйте данные значения только в том случае, если вы знаете к чему это приведет!<br/>В случае неверных действий, вы можете нанести непоравимые нарушения в работе сайта.');

// V 1.0.12
define('_COM_SEF_USE_CAT_ALIAS', 'Использовать алиасы Категории');
define('_COM_SEF_TT_USE_CAT_ALIAS', 'Если <strong>Да</strong>, то sh404sef будет использовать алиас Категории вместо ее актуального названия каждый раз, когда оно потребуется для создания ссылки.');
define('_COM_SEF_USE_SEC_ALIAS', 'Использовать алиасы Раздела');
define('_COM_SEF_TT_USE_SEC_ALIAS', 'Если <strong>Да</strong>, то sh404sef будет использовать алиас Раздела вмето его актуального названия каждый раз, когда оно потребуется для создания ссылки.');
define('_COM_SEF_USE_MENU_ALIAS', 'Использовать алиасы Меню');
define('_COM_SEF_TT_USE_MENU_ALIAS', 'Если <strong>Да</strong>, то sh404sef будет использовать алиас элемента Меню вместо его актуального заголовка каждый раз, когда он понадобится для создания ссылки.');
define('_COM_SEF_SH_ENABLE_TABLE_LESS', 'Использовать безтабличный вывод');
define('_COM_SEF_TT_SH_ENABLE_TABLE_LESS', 'Если <strong>Да</strong>, то sh404sef заставит использовать Joomla только div теги (т.е. никаких тегов table) при выводе материалов в соответствии шаблону, который вы используете. Шаблон Beez при этом необходимо оставить, чтобы все работало корректно. Он установлен по умолчанию.<br /><strong>ВНИМАНИЕ</strong>: необходимо будет подстроить ваш шаблон для соответствия таком (div) формату вывода.');

// V 1.0.13
define( '_COM_SEF_JC_MODULE_CACHING_DISABLED', 'Кэширование для языкового модуля Joomfish было отключено!');

// V 1.5.3
define('_COM_SEF_SH_ALWAYS_APPEND_ITEMS_PER_PAGE', 'Всегда добавлять постранично элементы #items');
define('_COM_SEF_TT_SH_ALWAYS_APPEND_ITEMS_PER_PAGE', 'Если <strong>Да</strong>, то sh404sef всегда будет добавлять номер постраничных элементов в пронумерованных ссылках. Например, .../Page-2.html станет .../Page2-10.html, если данная настройка приведет к отображению 10-ти элементов на страницу. Данное отображение необходимо, например, если используются выпадающие списки, чтобы пользователи могли выбрать кол-во отображаемых элементов на страницу.');
define('_COM_SEF_SH_REDIRECT_CORRECT_CASE_URL', 'Перенаправление 301 для исправления регистра');
...
define('_COM_SEF_TT_JOOMLA_LIVE_SITE', 'Здесь вы видите основную ссылку вашего веб сайта. Например:<br />http://www.example.com<br/>или<br/> http://example.com<br />(без завершающего слэша)<br />Это не sh404sef установка, а скорее <b>Joomlы</b>. Она сохранена в собственном конфигурационном файле Joomlы (configuration.php).<br />Обычно Joomla нормально определяет основной адрес сайта автоматически. Однако, если отобажаемый здесь адрес неверен, задайте его вручную самостоятельно. Сделать это можно изменением содержимого configuration.php (обычно, используя FTP).<br/>Симптомами неверного значения являются: не оторажаются шаблон или изображения, не функционируют кнопки, потеряны все стили (цвета, шрифты и т.д.), или подобное...');
define('_COM_SEF_TT_JOOMLA_LIVE_SITE_MISSING', 'ВНИМАНИЕ: $live_site потерян из файла configuration.php Joomla или же не начинается с "http://" или "https://" !');
define('_COM_SEF_SH_JCL_INSERT_EVENT_ID', 'Вставлять ID события');
define('_COM_SEF_TT_SH_JCL_INSERT_EVENT_ID', 'Если Да, внутренний ID события будет присоединен к заголовку события в ссылках, чтобы сделать их уникальными.');
define('_COM_SEF_SH_JCL_INSERT_CATEGORY_ID', 'Вставлять ID катигории');
define('_COM_SEF_TT_SH_JCL_INSERT_CATEGORY_ID', 'Если Да, то при присутствии категории в ссылке, внутренний ID категории будет присоединен, чтобы сделать ее уникальной.');
define('_COM_SEF_SH_JCL_INSERT_CALENDAR_ID', 'Вставлять ID календаря');
define('_COM_SEF_TT_SH_JCL_INSERT_CALENDAR_ID', 'Если Да, то при использовании названия календаря, внутренний ID календаря будет присоединен, чтобы сделать его уникальным.');
define('_COM_SEF_SH_JCL_INSERT_CALENDAR_NAME', 'Вставлять название Календаря');
define('_COM_SEF_TT_SH_JCL_INSERT_CALENDAR_NAME', 'Если Да, то все ссылки, где задан определенный календарь, будут иметь название календаря в ссылке. Если календарь в ссылке не определен, то будет добавлен заголовок меню.');
define('_COM_SEF_SH_JCL_INSERT_DATE', 'Вставлять Дату');
define('_COM_SEF_TT_SH_JCL_INSERT_DATE', 'Если Да, то дата целевой страницы будет добавляться к каждой ссылке.');
define('_COM_SEF_SH_JCL_INSERT_DATE_IN_EVENT_VIEW', 'Вставлять дату в ссылку События');
define('_COM_SEF_TT_SH_JCL_INSERT_DATE_IN_EVENT_VIEW', 'Если Да, то каждая дата события будет присоединяться к ссылкам на странице детализации события.');
define('_COM_SEF_SH_JCL_TITLE', 'Конфигурация JCal Pro');
define('_COM_SEF_SH_PAGE_TITLE_TITLE', 'Конфигурация Заголовка страницы');
define('_COM_SEF_SH_CONTENT_TITLE_TITLE', 'Конф. стр. Заголовка содержимого Joomla');
define('_COM_SEF_SH_CONTENT_TITLE_SHOW_SECTION', 'Вставлять Раздел');
define('_COM_SEF_TT_CONTENT_TITLE_SHOW_SECTION', 'Если Да, то раздел статьи будет вставлен в заголовок страницы статьи');
define('_COM_SEF_SH_CONTENT_TITLE_SHOW_CAT', 'Вставлять Категорию');
define('_COM_SEF_TT_CONTENT_TITLE_SHOW_CAT', 'Если Да, то категория раздела будет вставлена в заголовок страницы статьи');
define('_COM_SEF_SH_CONTENT_TITLE_USE_ALIAS', 'Исп. алиас заголовка статьи');
define('_COM_SEF_TT_CONTENT_TITLE_USE_ALIAS', 'Если Да, алиас статьи будет использован в заголовке страницы вместо актуального заголовка статьи.');
define('_COM_SEF_SH_CONTENT_TITLE_USE_CAT_ALIAS', 'Исп. алиас Категории');
define('_COM_SEF_TT_CONTENT_TITLE_USE_CAT_ALIAS', 'Если Да, то алиас категории будет использован в заголовке страницы вместо актуального заголовка категории.');
define('_COM_SEF_SH_CONTENT_TITLE_USE_SEC_ALIAS', 'Исп. алиас Раздела');
define('_COM_SEF_TT_CONTENT_TITLE_USE_SEC_ALIAS', 'Если Да, то алиас раздела будет использован в заголовке страницы вместо актуального заголовка раздела.');
define('_COM_SEF_SH_PAGE_TITLE_SEPARATOR', 'Разделитель заголовка стр.');
define('_COM_SEF_TT_SH_PAGE_TITLE_SEPARATOR', 'Введите символ или строку для отделения различных частей заголовка стр. По умолчанию символ | окружен единичным отступом.');

// V 1.5.7
define('_COM_SEF_DISPLAY_DUPLICATE_URLS_TITLE', 'Дубликаты');
define('_COM_SEF_DISPLAY_DUPLICATE_URLS_NOT', 'Отображать только основную ссылку');
define('_COM_SEF_DISPLAY_DUPLICATE_URLS', 'Отображать основную и дублированную ссылки');
define('_COM_SEF_SH_INSERT_ARTICLE_ID_TITLE', 'Вставить ID статьи в ссылку');
define('_COM_SEF_TT_SH_INSERT_ARTICLE_ID_TITLE', 'Если <strong>Да</strong>, то внутренний ID статьи будет добавлен к заголовку статьи в ссылках, чтобы быть уверенным в том, что каждая из статей может быть доступна индивидуально, т.е. даже, если 2 статьи имеют одинаковые заголовки articles, или же заголовки, создающие одинаковые ссылки (после очистки от неверных символов и подобного). Данный ID привнесет не СЕО значение и лучше убедиться, что у вас нет статей с идентичным заголовком в том же разделе и категории.<br />Если же ввод статей вам неподконтролен, данная настройка поможет быть уверенным в том, что статьи будут доступны articles в угоду хорошей поисковой оптимизации.');

// V 1.5.8

define('_COM_SEF_SH_JS_TITLE', 'Конфигурация JomSocial ');
define('_COM_SEF_SH_JS_INSERT_NAME', 'Вставлять Имя Jomsocial');
define('_COM_SEF_TT_SH_JS_INSERT_NAME', 'Если <strong>Да</strong>, элемент заголовка меню предшествующий JomSocial главной страницы, будет добавлен ко всем SEF ссылкам JomSocial.');
define('_COM_SEF_SH_JS_INSERT_USER_NAME', 'Вставлять Краткое имя польз.');
define('_COM_SEF_TT_SH_JS_INSERT_USER_NAME', 'Если <strong>Да</strong>, краткое имя пользователя будет добавлено в SEF ссылки. <strong>ВНИМАНИЕ</strong>: это может послужить причиной серьезного увеличения размера БД и затормозить работу сайта при наличии большого числа зарегистрированных пользователей.');
define('_COM_SEF_SH_JS_INSERT_USER_FULL_NAME', 'Вставлять Полное имя польз.');
define('_COM_SEF_TT_SH_JS_INSERT_USER_FULL_NAME', 'Если <strong>Да</strong>, молное имя пользователя будет добавлено в SEF ссылки. <strong>ВНИМАНИЕ</strong>: это может послужить причиной серьезного увеличения размера БД и затормозить работу сайта при наличии большого числа зарегистрированных пользователей.');
define('_COM_SEF_SH_JS_INSERT_GROUP_CATEGORY', 'Вставлять Категорию группы');
define('_COM_SEF_TT_SH_JS_INSERT_GROUP_CATEGORY', 'Если <strong>Да</strong>, категория групп пользователей будет добавлена в SEF ссылки туда, где используется имя группы.');
define('_COM_SEF_SH_JS_INSERT_GROUP_CATEGORY_ID', 'Вставлять ID категории группы');
define('_COM_SEF_TT_SH_JS_INSERT_GROUP_CATEGORY_ID', 'Если <strong>Да</strong>, ID категории группы пользователей будет добавлен к имени категории, <strong>когда предыдущий параметр также установлен в Да</strong>, в случае, если две категории имеют сходные названия.');
define('_COM_SEF_SH_JS_INSERT_GROUP_ID', 'Вставлять ID группы');
define('_COM_SEF_TT_SH_JS_INSERT_GROUP_ID', 'Если <strong>Да</strong>, ID группы пользователей будет добавлен к названию группы, в случае, если две группы имеют сходные названия.');
define('_COM_SEF_SH_JS_INSERT_GROUP_BULLETIN_ID', 'Вставлять ID бюллетеня гуппы');
define('_COM_SEF_TT_SH_JS_INSERT_GROUP_BULLETIN_ID', 'Если <strong>Да</strong>, ID бюллетеня группы пользователей будет добавлен к названию бюллетеня, в случае, если два бюллетеня имеют сходные названия.');
define('_COM_SEF_SH_JS_INSERT_DISCUSSION_ID', 'Вставлять ID группы обсуждения');
define('_COM_SEF_TT_SH_JS_INSERT_DISCUSSION_ID', 'Если <strong>Да</strong>, ID группы обсуждения пользователей будет добавлен к названию обсуждения, в случае, если две дискуссии имеют сходные названия.');
define('_COM_SEF_SH_JS_INSERT_MESSAGE_ID', 'Вставлять ID сообщения');
define('_COM_SEF_TT_SH_JS_INSERT_MESSAGE_ID', 'Если <strong>Да</strong>, ID сообщения будет добавлен к названию сообщения, в случае, если два сообщения имеют сохдные названия.');
define('_COM_SEF_SH_JS_INSERT_PHOTO_ALBUM', 'Вставлять Имя фотоальбома');
define('_COM_SEF_TT_SH_JS_INSERT_PHOTO_ALBUM', 'Если <strong>Да</strong>, название альбома к которому оно относится и будет вставлено в SEF ссылки фотографии или партии фотографий.');
define('_COM_SEF_SH_JS_INSERT_PHOTO_ALBUM_ID', 'Вставлять ID фотоальбома');
define('_COM_SEF_TT_SH_JS_INSERT_PHOTO_ALBUM_ID', 'Если <strong>Да</strong>, ID альбома, которое будет добавлено к его названию, в случае, если два альбома имеют сходное название.');
define('_COM_SEF_SH_JS_INSERT_PHOTO_ID', 'Вставлять ID фотографии');
define('_COM_SEF_TT_SH_JS_INSERT_PHOTO_ID', 'Если <strong>Да</strong>, ID фотографии будет добавлено к ее названию, в случае, если две фотографии имеют сходные названия.');
define('_COM_SEF_SH_JS_INSERT_VIDEO_CAT', 'Вставлять Имя категории видео');
define('_COM_SEF_TT_SH_JS_INSERT_VIDEO_CAT', 'Если <strong>Да</strong>, название категории к которой оно относится и будет вставлено в SEF ссылки видео или партии видеофрагментов.');
define('_COM_SEF_SH_JS_INSERT_VIDEO_CAT_ID', 'Вставлять ID категории видео');
define('_COM_SEF_TT_SH_JS_INSERT_VIDEO_CAT_ID', 'Если <strong>Да</strong>, ID категории видео, которое будет добавлено к названию категории, в случае, если категории имеют сходные названия.');
define('_COM_SEF_SH_JS_INSERT_VIDEO_ID', 'Вставлять ID видео');
define('_COM_SEF_TT_SH_JS_INSERT_VIDEO_ID', 'Если <strong>Да</strong>, ID видео, которое будет добавлено к названию видео, в случае, если два видеофграгмента имеют сходные названия.');
define('_COM_SEF_SH_FB_INSERT_USERNAME', 'Вставлять Имя польз.');
define('_COM_SEF_TT_SH_FB_INSERT_USERNAME', 'Если <strong>Да</strong>, имя пользователя будет добавлено в SEF ссылки постов или профилей.');
define('_COM_SEF_SH_FB_INSERT_USER_ID', 'Вставлять ID польз.');
define('_COM_SEF_TT_SH_FB_INSERT_USER_ID', 'Если <strong>Да</strong>, ID пользователя будет добалвено к его имени, если предыдущий параметр установлен в Да, в случае, если два пользователя имеют сходное имя.');
define('_COM_SEF_SH_PAGE_NOT_FOUND_ITEMID', 'Элемент для стр. 404');
define('_COM_SEF_TT_SH_PAGE_NOT_FOUND_ITEMID', 'Если значение, введенно здесь, если не равно 0, то будет отображено на странице 404. Joomla будет использовать Itemid для определения шаблона и модулей для отображения. Itemid представляет элемент меню, таким образом вы сможете определить Itemids в перечне меню.');
//define('', '');
