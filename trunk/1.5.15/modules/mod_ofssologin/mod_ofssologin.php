...
if($user->get('aid') == 0)
{ //login
  $loginPost = ("$baseUrl/sso/user/login?return_url=/of/user/dashboard");
  $singupPost = ("$baseUrl/sso/user/signup");
...
{ //logout
  $logoutPost   = "$baseUrl/sso/user/logout";
  $dashboard    = "$baseUrl/of/user/dashboard";
  $ssoedit      = "$baseUrl/sso/user/edit";
  $txtusername  = $user->get('username');

  echo JText::_('HI');
  echo '<a href="'.$baseUrl.'/user/userprofile/">';
  echo $txtusername.'</a>';
  echo $pipe;
...
<div class="ofssologin_search_{$class_sfx}">
  <form id="of_search" action="{$baseUrl}/of/openfoundry/search" method="get" onsubmit="of_search(); return false;">
