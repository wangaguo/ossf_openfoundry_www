...
//			}
	...
	function _buildSortLists()
	{
		...
		if ($elsettings->showtitle == 1) {
		  $sortselects[]	= JHTML::_('select.option', JText::_('TITLE'), $elsettings->titlename );
		}
		
	    if ($elsettings->showlocate == 1) {
			$sortselects[] 	= JHTML::_('select.option', JText::_('VENUE'), $elsettings->locationname );
  	  	}
  	  	
    	if ($elsettings->showcity == 1) {
			$sortselects[] 	= JHTML::_('select.option', JText::_('CITY'), $elsettings->cityname );
    	}
    	
		if ($elsettings->showcat) {
			$sortselects[] 	= JHTML::_('select.option', JText::_('TYPE'), $elsettings->catfroname );
		}
		$sortselect 	= JHTML::_('select.genericlist', $sortselects, JText::_('filter_type'), 'size="1" class="inputbox"', 'value', 'text', $filter_type );
