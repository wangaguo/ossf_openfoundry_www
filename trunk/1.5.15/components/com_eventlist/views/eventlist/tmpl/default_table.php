...
<table class="eventtable" width="<?php echo $this->elsettings->tablewidth; ?>" border="0" cellspacing="0" cellpadding="0" summary="eventlist">
	...
	<thead>
			<tr>
				<th id="el_date" class="sectiontableheader" align="left"><?php echo JHTML::_('grid.sort', JText::_('DATE_1'), 'a.dates', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
				<?php
				if ($this->elsettings->showtitle == 1) :
				?>
				<th id="el_title" class="sectiontableheader" align="left"><?php echo JHTML::_('grid.sort', JText::_('TITLE'), 'a.title', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
				<?php
				endif;
				if ($this->elsettings->showlocate == 1) :
				?>
				<th id="el_location" class="sectiontableheader" align="left"><?php echo JHTML::_('grid.sort', JText::_('LOCATION'), 'l.venue', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
				<?php
				endif;
	...
	if ($this->noevents == 1) :
		...
		<?php
					...
					$this->rows =& $this->getDisableDate_rows();
		...
		?>
    			...
    			<td headers="el_date" align="left">
    				<strong>
    					...
    					<?php
    					if ($row->enddates && $row->dates != $row->enddates) :
				...
				<td headers="el_title" align="left" valign="top"><a href="<?php echo $detaillink ; ?>"> <?php echo $this->escape($row->title); ?></a>	
					<?php if ($startdate >$today){
									echo "<img src = '/components/com_eventlist/assets/images/new_event.png' style='float:right;'/> ";
					}
				...
				?>  
					...
					<td headers="el_location" align="left" valign="top">
						...
						echo $row->locid != 0 ? "<a href='".JRoute::_('index.php?view=venueevents&id='.$row->venueslug)."'>".$this->escape($row->city)."</a>" : '-';
