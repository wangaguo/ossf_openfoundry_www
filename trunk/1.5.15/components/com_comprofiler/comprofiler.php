...
*/
	...
	if ( $whereFields ) {
		...
		$tablesWhereSQL[]	=	'((lower(ue.cb_nickname) LIKE \'%'.$searchSQL.'%\') OR (lower(ue.cb_aboutme) LIKE \'%'.$searchSQL.'%\') OR (lower(u.name) LIKE \'%'.$searchSQL.'%\') OR (lower(u.username) LIKE \'%'.$searchSQL.'%\') OR (ue.cb_itskills LIKE \'%'.$searchSQL.'%\'))';
/*
		if ( $search === null ) {
			$search			=	'';
		}
