...
function uddeIMshowOutbox($myself, $item_id, $limit, $limitstart, $cryptpass, $config, $filter_user, $filter_unread) {
	...
	$allmessages = uddeIMselectOutbox($myself, $limitstart, $limit, $config, $filter_user, $filter_unread);
	...
	$referlink = "index.php?option=com_uddeim&task=outbox&Itemid=".$item_id.$addlink;
	if($total>$limit) {
		...
		echo "[<a href='".uddeIMsefRelToAbs("index.php?option=com_uddeim&task=outbox&Itemid=".$item_id."&limitstart=0&limit=".$total.$addlink)."'>"._UDDEIM_SHOWALL."</a>]";
		echo "</div>\n";
	}
