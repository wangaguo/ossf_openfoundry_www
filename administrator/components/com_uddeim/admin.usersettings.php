...
//  `autoresponder` int(1) NOT NULL default '0',  	0=no, 1=yes, 2=admin only
//  `autoforward` int(1) NOT NULL default '0',  	0=no, 1=yes, 2=admin only
...
function uddeIMshowUsersettings($option, $task, $act, $config) {
	...
	if($f_param[7]!="") $f_where[] = "a.autoforward='$f_param[6]'";
...
//	}
	...
	echo "<form name='adminForm' method='post' action='index2.php' class='adminForm' style='text-align:left;'>";
	echo '<input onclick="document.adminForm.autorespondercheck.checked ? document.adminForm.autorespondertext.disabled=false : document.adminForm.autorespondertext.disabled=true;" type="checkbox" '.$emn_responder_checkstatus.' name="autorespondercheck" />'._UDDEIM_EMN_AUTORESPONDER.'<br />';
...
function uddeIMeditAutoforward($option, $task, $act, $config) {
	...
	echo "<form name='adminForm' method='post' action='index2.php' class='adminForm' style='text-align:left;'>";
	echo '<input onclick="document.adminForm.autoforwardcheck.checked ? document.adminForm.autoforwardid.disabled=false : document.adminForm.autoforwardid.disabled=true;" type="checkbox" '.$emn_forward_checkstatus.' name="autoforwardcheck" />'._UDDEIM_EMN_AUTOFORWARD.'<br />';
//	echo "<textarea name='autoforwardid' class='inputbox' rows='1' cols='6'".($ison==1 ? '' : 'disabled="disabled"').">".htmlentities($autoforwardid,ENT_QUOTES, $config->charset)."</textarea><br />";
	uddeIMdoShowAllUsers(0, 25, $config, 2, $ison, $autoforwardid);		// show all users, I am an admin, $config, mode=2 forwarding box, enabled=$ison, selected name=$autoforwardid
	echo "<br />";
	// echo '<input type="submit" name="reply" class="button" value="'._UDDEIM_SAVECHANGE.'" />';
	echo '<input type="hidden" name="option" value="'.$option.'" />';
	echo '<input type="hidden" name="task" value="'.$task.'" />';
