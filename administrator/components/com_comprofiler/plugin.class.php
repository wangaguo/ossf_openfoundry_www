...
class cbFieldHandler extends cbPluginHandler  {
	...
	*/
	 ...
	 * @param  string                $reason      'profile' for user profile view, 'edit' for profile edit, 'register' for registration, 'search' for searches
	 * @param  int                   $list_compare_types   IF reason == 'search' : 0 : simple 'is' search, 1 : advanced search with modes
	 * @return mixed                
	 */
	...
	function bindSearchCriteria( &$field, &$searchVals, &$postdata, $list_compare_types, $reason ) {
		$query							=	array();
		$searchMode						=	$this->_bindSearchMode( $field, $searchVals, $postdata, $list_compare_types );
		if ( $searchMode ) {
			foreach ( $field->getTableColumns() as $col ) {
				$value					=	cbGetParam( $postdata, $col );
...
*/
		...
		switch ( $output ) {
			...
			case 'htmledit':
				...
				if ( $htmlspecialchars ) {
					return sprintf( '%s %s %s', '<span class="titleCell">' . $title . '</span>', '<span class="cbIs">' . 'is now' . '</span>', '<span class="fieldCell">' . htmlspecialchars( $value ) . '</span>' );
				} else {
					return sprintf( '%s is now %s', $title, $value );		//FIXME : LANGUAGE STRINGS FOR THIS !
				}
				break;
	...
	function _bindSearchMode( &$field, &$searchVals, &$postdata, $list_compare_types ) {
		switch ($list_compare_types) {
			case 1:
